...
<div class="main">
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      <div class="topbar-title">{{ __('Blood Inventory') }}</div>
    </div>
  ...
  <div class="content">
    ...
    <div class="page-hd">
      ...
      <h1>{{ __('Blood Inventory Overview') }}</h1>
      <div class="page-sub">{{ __('Recorded stock levels across every hospital on the platform — read-only; only each hospital can update its own stock.') }}</div>
    </div>
    ...
    <div class="stat-row">
      <div class="sc">
        ...
        <div class="sc-label">{{ __('Total Units (All Hospitals)') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
        ...
        <div class="sc-label">{{ __('Hospitals with Low/Critical Stock') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
      ...
      <div class="sc">
        ...
        <div class="sc-label">{{ __('Hospitals Tracked') }}</div>
      </div>
    </div>
    ...
    @if($criticalGroups->isNotEmpty())
      <div style="background:var(--warning-b);border:1px solid var(--warning-dark);border-radius:10px;padding:0.85rem 1.25rem;margin-bottom:1.25rem;font-size:0.85rem;color:var(--warning-dark);">
        <strong>{{ __('Critical:') }}</strong> {{ $criticalGroups->join(', ') }} {{ $criticalGroups->count() > 1 ? __('are') : __('is') }} {{ __('critically low across one or more hospitals.') }}
      </div>
    @endif
    ...
    <form method="GET" action="{{ route('admin.inventory.index') }}">
      <div class="filter-bar">
        ...
        <a href="{{ route('admin.inventory.index') }}" class="btn-reset">{{ __('Reset') }}</a>
      </div>
    </form>
        ...
        <table>
          ...
          <tbody>
            @forelse($inventory as $item)
              <tr>
                <td>
                  <a href="{{ route('admin.hospitals.show', $item->hospital_id) }}" class="hosp-link">{{ $item->hospital?->name ?? __('Deleted hospital') }}</a>
                </td>
            ...
            @empty
              <tr>
                <td colspan="7">
                  <div class="empty-state">{{ __('No inventory records match these filters.') }}</div>
                </td>
              </tr>
            @endforelse
          </tbody>
