...
<div class="main">
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      <div class="topbar-title">{{ __('Donor Profile') }}</div>
    </div>
    ...
    <div class="page-grid">
      ...
      <div>
        <div class="profile-card">
          ...
          <div class="pc-body">
            ...
            <div class="pc-row">
              <span class="pc-key">{{ __('Donation Center') }}</span>
          ...
          <div class="action-btns">
            ...
            <form method="POST" action="{{ route('admin.donors.toggle', $donor->id) }}" style="margin:0;">
              ...
              <button type="submit"
                ...
                {{ $donor->is_eligible ? __('Mark as Not Eligible') : __('Mark as Eligible') }}
              </button>
            </form>
            ...
            <form method="POST" action="{{ route('admin.donors.destroy', $donor->id) }}" style="margin:0;">
              ...
              <button type="submit"
                      ...
                      onclick="return confirm('{{ __('Permanently delete') }} {{ $donor->full_name }}? {{ __('This cannot be undone.') }}')">
      ...
      <div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('Health Information') }}</div>
              <div class="card-s">{{ __('Registered health data used for AI prediction') }}</div>
            </div>
          </div>
          <div class="info-grid">
            <div class="info-item">
              <div class="info-key">{{ __('Age') }}</div>
              <div class="info-val large">{{ $age > 0 ? $age : '—' }}<span style="font-size:0.9rem;font-family:'DM Sans';font-weight:300;">{{ $age > 0 ? ' '.__('yrs') : '' }}</span></div>
            </div>
            <div class="info-item">
              <div class="info-key">{{ __('Weight') }}</div>
            ...
            <div class="info-item">
              <div class="info-key">{{ __('Hemoglobin') }}</div>
            ...
            <div class="info-item">
              <div class="info-key">{{ __('Total Donations') }}</div>
              <div class="info-val large">{{ $donor->total_donations ?? 0 }}</div>
            </div>
            <div class="info-item">
              <div class="info-key">{{ __('Date of Birth') }}</div>
              <div class="info-val">{{ $donor->date_of_birth ? \Carbon\Carbon::parse($donor->date_of_birth)->format('d M Y') : '—' }}</div>
            </div>
            <div class="info-item">
              <div class="info-key">{{ __('Last Donation') }}</div>
              <div class="info-val">{{ $donor->last_donation_date ? \Carbon\Carbon::parse($donor->last_donation_date)->format('d M Y') : __('Never') }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('AI Prediction Breakdown') }}</div>
              <div class="card-s">{{ $modelMetrics['model'] ?? 'AI' }} {{ __('model analysis') }}</div>
            </div>
            <span class="badge {{ $donor->is_eligible ? 'b-elig' : 'b-not' }}">
              {{ $donor->is_eligible ? __('Eligible') : __('Not Eligible') }}
            </span>
          ...
          <div style="background:var(--gray);border-radius:9px;padding:1rem 1.1rem;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.5rem;">
              <span style="font-size:0.78rem;font-weight:500;color:var(--text);">{{ __('AI Confidence Score') }}</span>
              <span style="font-size:0.78rem;font-weight:500;color:var(--text);">
                {{ $donor->ai_confidence > 0 ? number_format($donor->ai_confidence,2).'%' : __('Not calculated') }}
              </span>
            </div>
  ...
  {{ __('Model:') }} {{ $modelMetrics['model'] ?? __('unavailable') }} ·
  {{ __('Last AI check:') }}
...
<div style="margin-top:1rem;padding:1rem 1.1rem;background:{{ $donor->is_anomaly ? '#FFFBEB' : 'var(--green-bg)' }};border:1px solid {{ $donor->is_anomaly ? '#FDE68A' : 'var(--green-b)' }};border-radius:9px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
    <div style="display:flex;align-items:center;gap:7px;">
      ...
      <span style="font-size:0.78rem;font-weight:500;color:{{ $donor->is_anomaly ? '#92400E' : 'var(--green)' }};">
        {{ __('Anomaly Detection') }} — {{ $donor->is_anomaly ? __('Suspicious Profile') : __('Normal Profile') }}
      </span>
    </div>
  ...
  <div style="font-size:0.75rem;color:{{ $donor->is_anomaly ? '#92400E' : '#166534' }};line-height:1.5;">
    @if($donor->is_anomaly)
      {{ __("This donor's health data falls outside the normal range of registered donors.") }}
      {{ __('Admin review recommended. Anomaly score:') }} {{ number_format($donor->anomaly_score, 2) }}
    @else
      {{ __('Donor health profile is within normal range. No suspicious patterns detected.') }}
    @endif
  </div>
</div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('Medical Notes & Conditions') }}</div>
              <div class="card-s">{{ __('Additional health information provided by donor') }}</div>
            </div>
          </div>
          @if($donor->medical_condition && $donor->medical_condition !== '')
            <div style="margin-bottom:0.75rem;">
              <span style="font-size:0.72rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.06em;">{{ __('Condition') }}</span>
          ...
          <div class="notes-box {{ !$donor->medical_notes ? 'empty' : '' }}">
            {{ $donor->medical_notes ?? __('No additional health notes provided.') }}
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              ...
              <div class="card-s">{{ $donor->donations->count() }} {{ $donor->donations->count() !== 1 ? __('recorded donations') : __('recorded donation') }}</div>
            </div>
          </div>
            ...
            <div class="info-grid" style="grid-template-columns:1fr;gap:0;">
              @foreach($donor->donations as $d)
                <div class="pc-row">
                  ...
                  <span class="pc-val">{{ $d->blood_group ?? '—' }} · {{ $d->donation_center ?? '—' }} · {{ $d->units }} {{ $d->units !== 1 ? __('units') : __('unit') }}</span>
                </div>
              @endforeach
            </div>
          @else
            <div class="notes-box empty">{{ __('No donations recorded yet.') }}</div>
          @endif
          ...
          <form method="POST" action="{{ route('admin.donors.donations.store', $donor->id) }}" style="margin-top:1rem;display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:end;">
            @csrf
            <div class="field">
              <label style="font-size:0.72rem;color:var(--muted);">{{ __('Donation Date') }}</label>
            ...
            <div class="field">
              <label style="font-size:0.72rem;color:var(--muted);">{{ __('Units') }}</label>
              <input type="number" name="units" value="1" min="1" max="5"
                     style="width:100%;padding:0.5rem 0.7rem;border:1px solid var(--gray-b);border-radius:7px;font-size:0.82rem;">
            </div>
            <button type="submit" class="action-btn ab-toggle-on" style="white-space:nowrap;">{{ __('Record Donation') }}</button>
          </form>
