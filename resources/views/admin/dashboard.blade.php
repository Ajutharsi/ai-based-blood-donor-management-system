...
<div class="main">
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      <div class="topbar-title">{{ __('Dashboard Overview') }}</div>
    </div>
    <div class="topbar-right">
      ...
      <div class="notif-wrap">
        ...
        <div class="notif-dropdown" id="notifDropdown">
          ...
          @endforelse
          <div class="notif-dd-foot"><a href="{{ route('admin.notifications.index') }}">{{ __('View all →') }}</a></div>
        </div>
      </div>
...
@endif
    ...
    <div class="stat-grid">
      <div class="stat-card">
        <div class="stat-card-top">
          ...
          <span class="stat-trend trend-up">+{{ $stats['new_this_month'] }} {{ __('this month') }}</span>
        </div>
        <div class="stat-num">{{ $stats['total_donors'] }}</div>
        <div class="stat-label">{{ __('Total Donors') }}</div>
        <div class="stat-sub">{{ $stats['eligible_donors'] }} {{ __('eligible') }} · {{ $stats['not_eligible'] }} {{ __('not eligible') }}</div>
      </div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-label">{{ __('Eligible Donors') }}</div>
        <div class="stat-sub">{{ __('Ready to donate now') }}</div>
      </div>
      ...
      <div class="stat-card">
        <div class="stat-card-top">
          ...
          <span class="stat-trend trend-down">{{ $stats['not_eligible'] }} {{ __('not eligible') }}</span>
        </div>
        <div class="stat-num">{{ $stats['new_this_month'] }}</div>
        <div class="stat-label">{{ __('New This Month') }}</div>
        <div class="stat-sub">{{ __('Registered donors') }}</div>
      </div>
      ...
      <div class="stat-card">
        ...
        @if($modelMetrics)
          ...
          <div class="stat-label">{{ __('AI Accuracy') }}</div>
          <div class="stat-sub">
            {{ $modelMetrics['model'] }}
            @if(!empty($modelMetrics['test_set_rows']))
              · {{ __('held-out test on') }} {{ number_format($modelMetrics['test_set_rows']) }} {{ __('rows') }}
            @endif
            @if(!empty($modelMetrics['trained_at']))
              · {{ __('evaluated') }} {{ \Carbon\Carbon::parse($modelMetrics['trained_at'])->diffForHumans() }}
            @endif
          </div>
        @else
          <div class="stat-num" style="color:var(--muted);">—</div>
          <div class="stat-label">{{ __('AI Accuracy') }}</div>
          <div class="stat-sub">{{ __('Not available — AI service unreachable or not yet evaluated') }}</div>
        @endif
      </div>
    </div>
    ...
    <div class="grid-2">
      ...
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Blood Group Distribution') }}</div>
            <div class="card-sub">{{ __('Donor count by blood type') }}</div>
          </div>
          <a class="card-action" href="{{ route('admin.donors.index') }}">{{ __('View all →') }}</a>
        </div>
        <div class="bar-chart">
      ...
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Eligibility Status') }}</div>
            <div class="card-sub">{{ __('AI prediction breakdown') }}</div>
          </div>
        </div>
        @php
        ...
        <div class="donut-wrap">
          <svg width="110" height="110" viewBox="0 0 110 110" style="flex-shrink:0;">
            ...
            <text x="55" y="62" text-anchor="middle" font-family="DM Sans,sans-serif" font-size="8" fill="#6B3B40">{{ __('eligible') }}</text>
          </svg>
          <div class="donut-legend">
            <div class="dl-row"><span class="dl-dot" style="background:#1D4ED8"></span><span class="dl-label">{{ __('Eligible') }}</span><span class="dl-val">{{ $stats['eligible_donors'] }}</span></div>
            <div class="dl-row"><span class="dl-dot" style="background:#FDE68A"></span><span class="dl-label">{{ __('Not eligible') }}</span><span class="dl-val">{{ $stats['not_eligible'] }}</span></div>
            <div class="dl-row"><span class="dl-dot" style="background:#E4DEDE"></span><span class="dl-label">{{ __('Total') }}</span><span class="dl-val">{{ $stats['total_donors'] }}</span></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid-3-1">
      ...
      <div class="card" style="padding-bottom:0;">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Recent Donors') }}</div>
            <div class="card-sub">{{ __('Latest 5 registered donors') }}</div>
          </div>
          <a class="card-action" href="{{ route('admin.donors.index') }}">{{ __('View all →') }}</a>
        </div>
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ...
              @empty
                <tr>
                  <td colspan="6" class="empty-state">{{ __('No donors registered yet.') }}</td>
                </tr>
              @endforelse
            </tbody>
          </table>
        ...
        <div class="activity-list">
          @forelse($recent_donors as $donor)
            <div class="act-item">
              ...
              <div>
                <div class="act-text">
                  <strong>{{ $donor->first_name }}</strong> {{ __('registered') }}
                  {{ $donor->is_eligible ? '— '.__('Eligible ✓') : '— '.__('Not eligible') }}
                </div>
          ...
          @empty
            <div class="empty-state">{{ __('No recent activity.') }}</div>
          @endforelse
        </div>

        {{-- QUICK STATS --}}
        <div style="margin-top:1.25rem;padding-top:1.25rem;border-top:1px solid var(--border);">
          <div style="font-size:0.78rem;font-weight:500;color:var(--text);margin-bottom:0.75rem;">{{ __('Quick Stats') }}</div>
          <div style="display:flex;flex-direction:column;gap:8px;">
            <div style="display:flex;justify-content:space-between;font-size:0.775rem;">
              <span style="color:var(--muted);">O+ {{ __('donors') }}</span>
              <span style="font-weight:500;">{{ $bloodCounts['O+'] ?? 0 }}</span>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:0.775rem;">
              <span style="color:var(--muted);">A+ {{ __('donors') }}</span>
              <span style="font-weight:500;">{{ $bloodCounts['A+'] ?? 0 }}</span>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:0.775rem;">
              <span style="color:var(--muted);">{{ __('AI eligible rate') }}</span>
              <span style="font-weight:500;color:var(--green);">{{ $eligiblePct }}%</span>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:0.775rem;">
              <span style="color:var(--muted);">{{ __('New this month') }}</span>
    ...
    <div style="margin:1.75rem 0 0.75rem;">
      <span style="font-size:0.78rem;font-weight:500;color:var(--text);">{{ __('Advanced Analytics') }}</span>
      <div style="font-size:0.72rem;color:var(--muted);margin-top:2px;">{{ __('Platform-wide trends across the last 6 months') }}</div>
    </div>
    ...
    <div class="grid-2">
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Monthly Donations') }}</div>
            <div class="card-sub">{{ __('Completed donations recorded per month') }}</div>
          </div>
        </div>
        <div style="height:220px;"><canvas id="chartMonthlyDonations"></canvas></div>
      </div>

      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Blood Request Trends') }}</div>
            <div class="card-sub">{{ __('Total vs. fulfilled requests per month') }}</div>
          </div>
        </div>
        <div style="height:220px;"><canvas id="chartRequestTrends"></canvas></div>
      </div>
    </div>
    ...
    <div class="grid-2">
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Blood Inventory Trends') }}</div>
            <div class="card-sub">{{ __('Units added vs. removed per month (all hospitals)') }}</div>
          </div>
        </div>
      ...
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Blood Group Distribution') }}</div>
            <div class="card-sub">{{ __('Registered donors by blood group') }}</div>
          </div>
        </div>
        <div style="height:220px;"><canvas id="chartBloodGroupDist"></canvas></div>
      </div>
    </div>
    ...
    <div class="grid-2">
      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('Hospital Performance') }}</div>
            <div class="card-sub">{{ __('Top hospitals by request volume & fulfillment rate') }}</div>
          </div>
        </div>
        <div style="height:260px;"><canvas id="chartHospitalPerformance"></canvas></div>
      </div>

      <div class="card">
        <div class="card-header">
          <div>
            <div class="card-title">{{ __('AI Prediction Summary') }}</div>
            <div class="card-sub">{{ __('Volume and average confidence per prediction type') }}</div>
          </div>
        </div>
        <div style="height:260px;"><canvas id="chartAiPredictions"></canvas></div>
      </div>
    </div>
...
(function () {
  ...
  new Chart(document.getElementById('chartMonthlyDonations'), {
    type: 'line',
    data: {
      labels: monthlyDonations.map(function (r) { return r.label; }),
      datasets: [{ label: 'Donations', data: monthlyDonations.map(function (r) { return r.count; }), borderColor: THEME.secondary, backgroundColor: 'rgba(13,148,136,0.15)', fill: true, tension: 0.35 }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: commonScales }
  });

  var requestTrends = @json($bloodRequestTrends);
  new Chart(document.getElementById('chartRequestTrends'), {
    type: 'line',
    data: {
      labels: requestTrends.map(function (r) { return r.label; }),
      datasets: [
        { label: 'Total Requests', data: requestTrends.map(function (r) { return r.total; }), borderColor: THEME.primary, backgroundColor: 'rgba(29,78,216,0.12)', fill: true, tension: 0.35 },
        { label: 'Fulfilled', data: requestTrends.map(function (r) { return r.fulfilled; }), borderColor: THEME.green, backgroundColor: 'rgba(22,163,74,0.12)', fill: true, tension: 0.35 }
      ]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: commonScales }
  });

  var inventoryTrends = @json($bloodInventoryTrends);
  new Chart(document.getElementById('chartInventoryTrends'), {
    type: 'bar',
    data: {
      labels: inventoryTrends.map(function (r) { return r.label; }),
      datasets: [
        { label: 'Units Added', data: inventoryTrends.map(function (r) { return r.added; }), backgroundColor: THEME.secondary, borderRadius: 4 },
        { label: 'Units Removed', data: inventoryTrends.map(function (r) { return r.removed; }), backgroundColor: THEME.warning, borderRadius: 4 }
      ]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: commonScales }
  });

  var bgDist = @json($bloodGroupDistribution);
  new Chart(document.getElementById('chartBloodGroupDist'), {
    type: 'doughnut',
    data: {
      labels: bgDist.map(function (r) { return r.blood_group; }),
      datasets: [{
        data: bgDist.map(function (r) { return r.count; }),
        backgroundColor: ['#1D4ED8', '#0D9488', '#06B6D4', '#60A5FA', '#99F6E4', '#A5F3FC', '#1E3A8A', '#115E59']
      }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 12, padding: 10 } } } }
  });

  var hospPerf = @json($hospitalPerformance);
  new Chart(document.getElementById('chartHospitalPerformance'), {
    type: 'bar',
    data: {
      labels: hospPerf.map(function (r) { return r.hospital; }),
      datasets: [
        { label: 'Total Requests', data: hospPerf.map(function (r) { return r.total; }), backgroundColor: THEME.primary, borderRadius: 4 },
        { label: 'Fulfilled', data: hospPerf.map(function (r) { return r.fulfilled; }), backgroundColor: THEME.green, borderRadius: 4 }
      ]
    },
    options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: { x: { beginAtZero: true, grid: { color: THEME.grid } }, y: { grid: { display: false } } } }
  });

  var aiSummary = @json($aiPredictionSummary);
  new Chart(document.getElementById('chartAiPredictions'), {
    type: 'bar',
    data: {
      labels: aiSummary.map(function (r) { return r.type; }),
      datasets: [
        { label: 'Total Predictions', data: aiSummary.map(function (r) { return r.total; }), backgroundColor: THEME.accent, borderRadius: 4, yAxisID: 'y' },
        { label: 'Avg. Confidence %', data: aiSummary.map(function (r) { return r.avg_confidence; }), backgroundColor: THEME.warning, borderRadius: 4, yAxisID: 'y1' }
      ]
    },
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: true, position: 'left', grid: { color: THEME.grid } },
        y1: { beginAtZero: true, position: 'right', grid: { display: false }, max: 100 }
      }
    }
  });
})();
