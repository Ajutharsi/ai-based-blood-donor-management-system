<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .td-avatar{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:500;flex-shrink:0;background:var(--blue-bg);color:var(--blue);}
  ...
  .b-verified{background:#F0FDF4;color:#166534;}
...
<div class="main">
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      <div class="topbar-title">{{ __('Hospital Management') }}</div>
    </div>
  ...
  <div class="content">
    ...
    <div class="page-hd">
      <div class="page-hd-left">
        ...
        <h1>{{ __('All Hospitals') }}</h1>
      </div>
      <span style="font-size:0.82rem;color:var(--muted);">
        {{ $hospitals->total() }} {{ $hospitals->total() !== 1 ? __('hospitals found') : __('hospital found') }}
      </span>
    </div>
    ...
    <form method="GET" action="{{ route('admin.hospitals.index') }}">
      <div class="filter-bar">
        ...
        <select name="status" class="filter-select">
          ...
          <option value="pending"   {{ request('status') === 'pending'   ? 'selected' : '' }}>{{ __('Pending Verification') }}</option>
        </select>
        ...
        <a href="{{ route('admin.hospitals.index') }}" class="btn-reset">{{ __('Reset') }}</a>
      </div>
    </form>
    ...
    <div class="filter-tabs">
      <a href="{{ route('admin.hospitals.index') }}"
        ...
        {{ __('All') }} ({{ $hospitals->total() }})
      </a>
      <a href="{{ route('admin.hospitals.index', ['status' => 'verified']) }}"
         class="ftab {{ request('status') === 'verified' ? 'active' : '' }}">
        ✓ {{ __('Verified') }}
      </a>
    ...
    <div class="table-card">
        ...
        <table>
          ...
          <tbody>
            @forelse($hospitals as $hospital)
              <tr>
                <td style="color:var(--muted);font-size:0.75rem;">{{ $hospital->id }}</td>
                <td>
                  <div class="td-name">
                    <div class="td-avatar">
                    ...
                    <div>
                      <div class="td-name-text">{{ $hospital->name }}</div>
                      <div class="td-name-sub">{{ $hospital->email }}</div>
                    </div>
                  </div>
                </td>
                <td>{{ $hospital->registration_id ?? '—' }}</td>
                <td>{{ $hospital->district ?? '—' }}</td>
                <td>{{ $hospital->blood_requests_count }}</td>
                <td>
                ...
                <td>
                  <div class="actions">
                    {{-- VIEW --}}
                    <a href="{{ route('admin.hospitals.show', $hospital->id) }}" class="btn-view">{{ __('View') }}</a>

                    {{-- TOGGLE VERIFICATION --}}
                    <form method="POST" action="{{ route('admin.hospitals.toggle', $hospital->id) }}" style="margin:0;">
                      @csrf
                      <button type="submit"
                              class="btn-toggle {{ $hospital->is_verified ? 'btn-toggle-on' : 'btn-toggle-off' }}"
                              onclick="return confirm('{{ $hospital->is_verified ? __('Mark as unverified?') : __('Verify this hospital?') }}')">
                        {{ $hospital->is_verified ? __('Unverify') : __('Verify') }}
                      </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('admin.hospitals.destroy', $hospital->id) }}" style="margin:0;">
                      ...
                      <button type="submit" class="btn-delete"
                              onclick="return confirm('{{ __('Delete') }} {{ $hospital->name }}? {{ __('This cannot be undone.') }}')">
                        {{ __('Delete') }}
                      </button>
                    </form>
                  </div>
              ...
              <tr>
                <td colspan="8">
                  <div class="empty-state">
                    <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M12 8v8M8 12h8"/></svg>
                    <p>{{ __('No hospitals found matching your search.') }}</p>
                  </div>
                </td>
              </tr>
            @endforelse
      ...
      @if($hospitals->hasPages())
        <div class="pagination-wrap">
          <div class="page-info">
            {{ __('Showing') }} {{ $hospitals->firstItem() }}–{{ $hospitals->lastItem() }} {{ __('of') }} {{ $hospitals->total() }} {{ __('hospitals') }}
          </div>
          <div class="page-links">
            @if($hospitals->onFirstPage())
              <span>←</span>
            @else
              <a href="{{ $hospitals->previousPageUrl() }}">←</a>
            @endif
            ...
            @if($hospitals->hasMorePages())
              <a href="{{ $hospitals->nextPageUrl() }}">→</a>
            @else
              <span>→</span>
            @endif
          </div>
