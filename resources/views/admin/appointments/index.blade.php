...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="page-hd">
      ...
      <h1>Donation Appointment Overview</h1>
      <div class="page-sub">Read-only visibility into every hospital's appointments — only the owning hospital can approve, reject, reschedule, or complete.</div>
    </div>

    <div class="stat-row">
      <div class="sc"><div class="sc-num">{{ $stats['total'] }}</div><div class="sc-label">Total</div></div>
      <div class="sc"><div class="sc-num">{{ $stats['completed'] }}</div><div class="sc-label">Completed</div></div>
      <div class="sc"><div class="sc-num">{{ $stats['pending'] }}</div><div class="sc-label">Pending</div></div>
      <div class="sc"><div class="sc-num">{{ $stats['approved'] }}</div><div class="sc-label">Approved</div></div>
      <div class="sc"><div class="sc-num">{{ $stats['cancelled'] + $stats['rejected'] }}</div><div class="sc-label">Cancelled/Rejected</div></div>
    </div>

    <div class="grid-2">
      <div class="panel">
        <div class="panel-title">Monthly Appointment Volume</div>
        @php $maxTotal = max(1, $monthlyChart->max('total')); @endphp
        <div class="chart-row">
          @foreach($monthlyChart as $m)
            <div class="chart-col">
              <div class="chart-bar-wrap">
                <div class="chart-bar total" style="height:{{ $m['total'] > 0 ? max(6, ($m['total'] / $maxTotal) * 100) : 2 }}%;" title="{{ $m['total'] }} total"></div>
                <div class="chart-bar completed" style="height:{{ $m['completed'] > 0 ? max(6, ($m['completed'] / $maxTotal) * 100) : 2 }}%;" title="{{ $m['completed'] }} completed"></div>
              </div>
        ...
        <div class="chart-legend">
          <span><span class="legend-dot" style="background:var(--primary);"></span>Total booked</span>
          <span><span class="legend-dot" style="background:var(--secondary);"></span>Completed</span>
        </div>
      </div>
      <div class="panel">
        <div class="panel-title">By Blood Group</div>
        @forelse($byBloodGroup as $row)
          <div class="breakdown-row">
            <span class="breakdown-name">{{ $row['blood_group'] }}</span>
            <span class="breakdown-count">{{ $row['total'] }} total · {{ $row['completed'] }} completed</span>
          </div>
        @empty
    ...
    <div class="panel" style="margin-bottom:1.5rem;">
      <div class="panel-title">By Hospital</div>
    ...
    <div class="table-card">
          ...
          <tbody>
            @forelse($appointments as $a)
              <tr>
                ...
                <td><a href="{{ route('admin.hospitals.show', $a->hospital_id) }}" class="hosp-link">{{ $a->hospital?->name ?? 'Deleted hospital' }}</a></td>
                <td><a href="{{ route('admin.donors.show', $a->donor_id) }}" class="hosp-link">{{ $a->donor?->full_name ?? 'Deleted donor' }}</a></td>
            ...
            @empty
              <tr>
                <td colspan="7">
                  <div class="empty-state">No appointments match these filters.</div>
                </td>
              </tr>
            @endforelse
          </tbody>
      ...
      @if($appointments->hasPages())
        <div class="pagination-wrap">
          <div class="page-info">Showing {{ $appointments->firstItem() }}–{{ $appointments->lastItem() }} of {{ $appointments->total() }}</div>
