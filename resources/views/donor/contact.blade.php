...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    /* HERO */
    .hero{padding:130px 5% 70px;background:linear-gradient(160deg,#0F172A 0%,var(--primary-dark) 55%,#172554 100%);position:relative;overflow:hidden;text-align:center;}
    ...
    .contact-card:hover{border-color:var(--primary-mid);background:var(--primary-light);}
    ...
    .cc-icon svg{width:18px;height:18px;stroke:var(--primary);}
    ...
    /* EMERGENCY BOX */
    .emergency-box{background:linear-gradient(135deg,#1E293B,#0E7490);border-radius:14px;padding:1.5rem;margin-top:1.5rem;position:relative;overflow:hidden;}
    ...
    .em-tag{font-size:0.68rem;font-weight:700;color:#A5F3FC;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem;}
    ...
    .form-group label span{color:#D97706;}
    ...
    .form-group input:focus,.form-group select:focus,.form-group textarea:focus{border-color:var(--primary);background:white;box-shadow:0 0 0 3px rgba(29,78,216,0.08);}
    .form-group textarea{resize:vertical;min-height:120px;line-height:1.6;}
    .form-group .error{font-size:0.73rem;color:#D97706;margin-top:0.2rem;}
    .submit-btn{width:100%;padding:0.85rem;background:var(--primary);color:white;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:0.9rem;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:0.5rem;}
    .submit-btn:hover{background:var(--primary-dark);transform:translateY(-1px);box-shadow:0 6px 20px rgba(29,78,216,0.3);}
    ...
    .sp:hover,.sp.sel{background:var(--primary-light);border-color:var(--primary-mid);color:var(--primary);}
    ...
    .alert-error{background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:1rem 1.25rem;margin-bottom:1.5rem;font-size:0.82rem;color:#92400E;}
...
<nav>
  ...
  <div class="nav-links">
    ...
    <a href="/contact" class="active">{{ __('Contact') }}</a>
...
<!-- HERO -->
<div class="hero">
  <div class="hero-tag"><span></span> {{ __('Get in Touch') }}</div>
  <h1>{{ __("We're here to") }} <em>{{ __('help') }}</em></h1>
  <p>{{ __('Have a question, a partnership proposal, or need urgent blood donor support? Reach out — we respond within 24 hours.') }}</p>
</div>
  ...
  <div class="section-inner">
    <div class="contact-grid">
      ...
      <div class="info-panel">
        <h2>{{ __('Contact LifeLink') }}</h2>
        <p>{{ __("Whether you're a donor, hospital administrator, or researcher — we want to hear from you.") }}</p>
        ...
        <div class="contact-cards">
          <div class="contact-card">
            ...
            <div>
              ...
              <div class="cc-sub">{{ __('General enquiries & technical support') }}</div>
            </div>
          </div>
          <div class="contact-card">
            ...
            <div>
              ...
              <div class="cc-sub">{{ __('Mon–Fri, 8:00 AM – 5:00 PM') }}</div>
            </div>
          </div>
          <div class="contact-card">
            ...
            <div>
              ...
              <div class="cc-val">{{ __('National Blood Transfusion Service') }}</div>
              <div class="cc-sub">{{ __('Baseline Road, Colombo 08, Sri Lanka') }}</div>
            </div>
          </div>
          <div class="contact-card">
            ...
            <div>
              <div class="cc-label">{{ __('Response Time') }}</div>
              <div class="cc-val">{{ __('Within 24 hours') }}</div>
              <div class="cc-sub">{{ __('We aim to respond to all messages quickly') }}</div>
            </div>
          </div>
        </div>

        <div class="emergency-box">
          <div class="em-tag">🚨 {{ __('Blood Emergency') }}</div>
          <div class="em-title">{{ __('Need blood urgently?') }}</div>
          <div class="em-text">{{ __('For critical blood requests, contact the NBTS emergency line directly. Hospitals can submit urgent requests through the Hospital Dashboard.') }}</div>
      ...
      <div class="form-panel">
        <div class="form-title">{{ __('Send us a message') }}</div>
        <div class="form-sub">{{ __('Fill in the form below and our team will get back to you shortly.') }}</div>

        @if(session('contact_success'))
        <div class="alert-success">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          {{ __("Thank you! Your message has been sent. We'll reply within 24 hours.") }}
        </div>
        @endif
        ...
        <form method="POST" action="/contact/send">
          ...
          <div class="form-group">
            <label>{{ __('Your Role') }}</label>
            <select name="role">
              <option value="">{{ __('Select your role') }}</option>
              <option value="donor"     {{ old('role')==='donor'     ?'selected':'' }}>{{ __('Blood Donor') }}</option>
              <option value="hospital"  {{ old('role')==='hospital'  ?'selected':'' }}>{{ __('Hospital / Medical Staff') }}</option>
              <option value="researcher"{{ old('role')==='researcher'?'selected':'' }}>{{ __('Researcher / Academic') }}</option>
              <option value="other"     {{ old('role')==='other'     ?'selected':'' }}>{{ __('Other') }}</option>
            </select>
          </div>

          <div class="form-group">
            <div class="sp-label">{{ __('Subject') }}</div>
            <div class="subject-pills">
              ...
              <div class="sp {{ old('subject')===$subj?'sel':'' }}" onclick="selectSubject(this,'{{ $subj }}')">{{ __($subj) }}</div>
              @endforeach
            </div>
          ...
          <div class="form-group">
            ...
            <textarea name="message" placeholder="{{ __('Tell us how we can help you…') }}">{{ old('message') }}</textarea>
...
<div class="map-section">
  <div class="map-wrap">
    <h3>{{ __('NBTS Donation Centres') }}</h3>
    <p>{{ __('Find your nearest National Blood Transfusion Service centre across Sri Lanka.') }}</p>
