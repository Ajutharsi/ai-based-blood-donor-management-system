...
<nav>
  ...
  <div class="nav-btns" style="display:flex;align-items:center;gap:10px;">
...
<section class="hero">
  <div class="hero-left">
    <div class="hero-badge">
      ...
      {{ __('AI-Powered Donor Matching') }}
    </div>
    <h1>{{ __('Saving Lives with') }} <span>{{ __('Smarter') }}</span> {{ __('Blood Donation') }}</h1>
    <p>{{ __('An intelligent platform that connects hospitals with eligible blood donors instantly — powered by machine learning that predicts donor suitability with') }}
...
<a class="btn-hero btn-hero-primary" href="{{ route('donor.register')}}">{{ __('Donate Blood Today') }}</a>
<a class="btn-hero btn-hero-outline" href="{{ route('donor.login')}}">{{ __('Sign In →') }}</a>
  ...
  <div class="hero-right">
    ...
    <div class="hero-card">
      <div class="card-title">{{ __('AI Donor Eligibility — Live') }}</div>
      <div class="donor-item">
        ...
        <span class="donor-badge badge-eli">{{ __('Eligible') }}</span>
      </div>
      <div class="donor-item">
      ...
      <div class="donor-item">
        ...
        <span class="donor-badge badge-not">{{ __('Cooldown') }}</span>
      </div>
      <div class="ai-label">
        <div class="ai-icon">
        ...
        <div class="ai-text">
          <strong>
            ...
            @if(!empty($modelMetrics['accuracy']))
              · {{ number_format($modelMetrics['accuracy'], 2) }}% {{ __('accuracy') }}
            @endif
          </strong>
          {{ __('Prediction based on age, weight & hemoglobin') }}
        </div>
      </div>
    </div>
  </div>
...
<div class="stats">
  <div class="stat-item">
    ...
    <div class="stat-label">{{ __('Registered Donors') }}</div>
  </div>
  <div class="stat-item">
    <div class="stat-num">{{ !empty($modelMetrics['accuracy']) ? number_format($modelMetrics['accuracy'], 1) . '%' : '—' }}</div>
    <div class="stat-label">{{ __('AI Prediction Accuracy') }}</div>
  </div>
  <div class="stat-item">
    <div class="stat-num">{{ $hospitalCount ?? 0 }}</div>
    <div class="stat-label">{{ __('Partner Hospitals') }}</div>
  </div>
  <div class="stat-item">
    <div class="stat-num">8</div>
    <div class="stat-label">{{ __('Blood Groups Covered') }}</div>
  </div>
</div>
...
<section class="section">
  ...
  <div class="section-title">{{ __('From registration to donation') }}<br>{{ __('in four simple steps') }}</div>
  <div class="section-sub">{{ __('Our AI handles the complex matching so donors and hospitals can focus on what matters — saving lives.') }}</div>
  <div class="steps">
    <div class="step">
      <div class="step-num">1</div>
      <div class="step-line"></div>
      <div class="step-title">{{ __('Donor registers') }}</div>
      <div class="step-desc">{{ __('Fill in your age, blood group, weight, and health details. Takes under 2 minutes.') }}</div>
    </div>
    <div class="step">
      <div class="step-num">2</div>
      <div class="step-line"></div>
      <div class="step-title">{{ __('AI assesses eligibility') }}</div>
      <div class="step-desc">{{ __('Our') }} {{ $modelMetrics['model'] ?? 'AI' }} {{ __("model instantly predicts if you're eligible to donate.") }}</div>
    </div>
    <div class="step">
      <div class="step-num">3</div>
      <div class="step-line"></div>
      <div class="step-title">{{ __('Hospital requests blood') }}</div>
      <div class="step-desc">{{ __('A hospital enters the required blood group and the system finds matches automatically.') }}</div>
    </div>
    <div class="step">
      <div class="step-num">4</div>
      <div class="step-line"></div>
      <div class="step-title">{{ __('Donor is notified') }}</div>
      <div class="step-desc">{{ __('Eligible donors receive an instant SMS or email alert with donation details.') }}</div>
    </div>
  </div>
</section>
...
<section class="features-section">
  ...
  <div class="section-title">{{ __('Everything built for') }}<br>{{ __('modern healthcare') }}</div>
  <div class="features-grid">
    <div class="feature-card">
      <div class="feature-icon">
        <svg viewBox="0 0 24 24"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
      </div>
      <div class="feature-title">{{ __('AI Eligibility Prediction') }}</div>
      <div class="feature-desc">{{ __('Machine learning models trained on real donor data predict eligibility based on hemoglobin, weight, and age') }}
        @if(!empty($modelMetrics['accuracy']))
          {{ __('with') }} {{ number_format($modelMetrics['accuracy'], 1) }}% {{ __('accuracy') }}
        @endif
      .</div>
    ...
    <div class="feature-card">
      ...
      <div class="feature-title">{{ __('Smart Donor Matching') }}</div>
      <div class="feature-desc">{{ __('Hospitals get an instant ranked list of the most suitable nearby donors matching the required blood group.') }}</div>
      <span class="feature-tag">{{ __('Real-time Matching') }}</span>
    </div>
    <div class="feature-card">
      <div class="feature-icon">
        <svg viewBox="0 0 24 24"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>
      </div>
      <div class="feature-title">{{ __('Instant Notifications') }}</div>
      <div class="feature-desc">{{ __('Eligible donors are alerted automatically via SMS and email the moment a matching blood request is raised.') }}</div>
      <span class="feature-tag">{{ __('SMS + Email') }}</span>
    </div>
    <div class="feature-card">
      <div class="feature-icon">
        <svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
      </div>
      <div class="feature-title">{{ __('Admin Dashboard') }}</div>
      <div class="feature-desc">{{ __('Full oversight of donors, hospitals, blood requests, and AI predictions from a single control panel.') }}</div>
      <span class="feature-tag">{{ __('Role-based Access') }}</span>
    </div>
    <div class="feature-card">
      <div class="feature-icon">
        <svg viewBox="0 0 24 24"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
      </div>
      <div class="feature-title">{{ __('Donation History Tracking') }}</div>
      <div class="feature-desc">{{ __("Tracks every donor's donation history and enforces the 56-day cooldown period automatically to ensure donor safety.") }}</div>
      <span class="feature-tag">{{ __('Safety First') }}</span>
    </div>
    <div class="feature-card">
      <div class="feature-icon">
        <svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
      </div>
      <div class="feature-title">{{ __('Secure & Private') }}</div>
      <div class="feature-desc">{{ __('All donor health data is encrypted and stored securely. Role-based login for donors, hospitals, and admins.') }}</div>
      <span class="feature-tag">{{ __('Laravel Auth + MySQL') }}</span>
    </div>
  </div>
</section>
...
<section class="cta-section">
  ...
  <h2>{{ __('One donation saves three lives.') }}</h2>
  <p>{{ __('Join thousands of donors already registered on LifeLink. It only takes a minute to sign up and make a difference.') }}</p>
  <div class="cta-btns">
    <a href="{{ route('donor.register') }}" class="btn-cta-white">{{ __('Register as Donor') }}</a>
    <a href="{{ route('donor.login') }}" class="btn-cta-outline">{{ __('Donor Login →') }}</a>
  </div>
</section>
...
<footer>
  ...
  <div class="footer-copy">© 2026 {{ __('LifeLink Blood Donor Management System') }}</div>
