...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      <div>
        <div class="tb-title">{{ __('Blood Requests Near You') }}</div>
        <div class="tb-sub">{{ __('Requests compatible with your blood group,') }} {{ $donor->blood_group ?? 'N/A' }}</div>
      </div>
    </div>
    <div class="tb-right">
  ...
  <div class="content">
    ...
    @if(!$donor->is_eligible)
      <div class="empty-state">
        ...
        <h3>{{ __("Blood Requests Are Hidden While You're Not Eligible") }}</h3>
        <p>{{ __('Your AI eligibility check currently shows you as not eligible to donate, so matching requests are not shown here.') }}<br>
        {{ __('Update your health profile once your situation changes, and this page will update automatically.') }}</p>
      </div>
    @elseif($requests->isEmpty())
      <div class="empty-state">
        <svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
        <h3>{{ __('No Matching Requests Right Now') }}</h3>
        <p>{{ __('There are currently no open blood requests compatible with your blood group') }} ({{ $donor->blood_group }}).<br>
        {{ __('Check back later — this list updates as hospitals submit new requests.') }}</p>
      </div>
    @else
      <div class="req-list">
          ...
          <div class="req-card">
            <div class="req-top">
              ...
              <div class="req-info">
                ...
                <div class="req-meta">
                  {{ $r->units_needed }} unit{{ $r->units_needed > 1 ? 's' : '' }} {{ __('needed') }}
                  @if($r->ward) · {{ $r->ward }} @endif
                  @if($r->required_by) · {{ __('Needed by') }} {{ \Carbon\Carbon::parse($r->required_by)->format('d M Y') }} @endif
                </div>
                <div class="req-badges">
                  <span class="badge {{ $badgeClass }}">{{ ucfirst($r->urgency) }}</span>
                  @if($sameDistrict)
                    <span class="badge b-district">{{ __('Your district') }}</span>
            ...
            @if($myResponse)
              <div class="status-note {{ $myResponse->status === 'available' ? 'status-available' : 'status-not-available' }}">
                @if($myResponse->status === 'available')
                  {{ __("You said you're") }} <strong>{{ __('available') }}</strong> {{ __('for this request.') }}
                @else
                  {{ __("You said you're") }} <strong>{{ __('not available') }}</strong> {{ __('for this request.') }}
                @endif
                <a onclick="document.getElementById('resp-forms-{{ $r->id }}').style.display='flex'; this.parentElement.style.display='none';">{{ __('Change response') }}</a>
              </div>
              <div id="resp-forms-{{ $r->id }}" class="respond-row" style="display:none;">
                <form method="POST" action="{{ route('donor.requests.respond', $r->id) }}" style="flex:1;">
                  ...
                  <button type="submit" class="respond-btn yes" style="width:100%;">{{ __("I'm Available") }}</button>
                </form>
                <form method="POST" action="{{ route('donor.requests.respond', $r->id) }}" style="flex:1;">
                  @csrf
                  <input type="hidden" name="status" value="not_available">
                  <button type="submit" class="respond-btn no" style="width:100%;">{{ __('Not Available') }}</button>
                </form>
              </div>
            @else
