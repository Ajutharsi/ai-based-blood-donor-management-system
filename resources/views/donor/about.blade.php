...
<nav>
  ...
  <div class="nav-links">
    ...
    <a href="{{route('find-donors')}}">{{ __('Find Donors') }}</a>
    <a href="{{ route('hospital.login') }}">{{ __('Hospitals') }}</a>
...
<section class="hero">
  <div class="hero-inner">
    <div class="hero-tag"><span></span> {{ __('Our Story') }}</div>
    <h1>{{ __('Built to save') }} <em>{{ __('lives') }}</em> {{ __('with intelligence') }}</h1>
    <p>{{ __("LifeLink is Sri Lanka's first AI-powered blood donor management system — connecting eligible donors with hospitals faster, smarter, and more accurately than ever before.") }}</p>
  </div>
</section>
      ...
      <div class="mission-visual">
        <div class="mv-cards">
          <div class="mv-card">
            ...
            <div class="mv-label">{{ __('AI model accuracy using') }} {{ $modelMetrics['model'] ?? 'our current model' }} {{ __('on') }} {{ number_format($modelMetrics['trained_on_rows'] ?? 0) }} {{ __('real donor records — our core prediction engine.') }}</div>
          </div>
          <div class="mv-stat">
            ...
            <div><div class="mv-stat-val">{{ number_format($modelMetrics['trained_on_rows'] ?? 0) }}</div><div class="mv-stat-lbl">{{ __('Donor records trained on') }}</div></div>
          </div>
          <div class="mv-stat">
            ...
            <div><div class="mv-stat-val">{{ $hospitalCount ?? 0 }}</div><div class="mv-stat-lbl">{{ __('Partner hospitals across Sri Lanka') }}</div></div>
          </div>
        </div>
      </div>
      <div class="mission-text">
        <div class="section-tag">{{ __('Our Mission') }}</div>
        <h2 class="section-title">{{ __('Making every drop count') }}</h2>
        <p>{{ __("Blood shortages cost lives — not because donors don't exist, but because connecting the right donor to the right hospital at the right time has always been a manual, slow process.") }}</p>
        <p>{{ __('LifeLink changes that. Our AI analyses key health indicators in real time and predicts donor eligibility with') }}
          @if($accuracyWhole !== null)
            {{ $accuracyWhole }}.{{ $accuracyDecimal }}%
          @else
            {{ __('high') }}
          @endif
          {{ __('accuracy — so hospitals get matched to eligible donors in seconds, not hours.') }}</p>
        <p>{{ __('Built as a final-year research project at the University of Sri Lanka, LifeLink combines machine learning, modern web technology, and a passion for public health into one unified platform.') }}</p>
      </div>
    </div>
  </div>
</section>
...
<section>
  <div class="section-inner">
    <div class="section-tag">{{ __('What We Stand For') }}</div>
    <h2 class="section-title">{{ __('Our core values') }}</h2>
    <div class="values-grid">
      <div class="value-card">
        <div class="vc-num"><span>01</span></div>
        <div class="vc-title">{{ __('Accuracy First') }}</div>
        <div class="vc-text">{{ __('Every eligibility decision is backed by a trained machine learning model — not guesswork. We rigorously validated our model on a 70/30 train-test split before deployment.') }}</div>
      </div>
      <div class="value-card">
        <div class="vc-num"><span>02</span></div>
        <div class="vc-title">{{ __('Privacy by Design') }}</div>
        <div class="vc-text">{{ __("Donor health data is encrypted, never sold, and only used for eligibility matching. Your blood group and medical details are yours — we're just the bridge.") }}</div>
      </div>
      <div class="value-card">
        <div class="vc-num"><span>03</span></div>
        <div class="vc-title">{{ __('Speed & Reliability') }}</div>
        <div class="vc-text">{{ __('In emergencies, every minute matters. Our FastAPI prediction engine returns eligibility results in under 200ms — faster than a human can read a form.') }}</div>
      </div>
      <div class="value-card">
        <div class="vc-num"><span>04</span></div>
        <div class="vc-title">{{ __('Inclusive Access') }}</div>
        <div class="vc-text">{{ __("Built for Sri Lanka — all 25 districts, all NBTS donation centres, all 8 blood groups. Whether you're in Colombo or Jaffna, LifeLink works for you.") }}</div>
      </div>
      <div class="value-card">
        <div class="vc-num"><span>05</span></div>
        <div class="vc-title">{{ __('Open Science') }}</div>
        <div class="vc-text">{{ __('Our models, training data structure, and evaluation metrics are documented openly. We believe in reproducible research and transparent AI.') }}</div>
      </div>
      <div class="value-card">
        <div class="vc-num"><span>06</span></div>
        <div class="vc-title">{{ __('Human Impact') }}</div>
        <div class="vc-text">{{ __("Each donation can save up to 3 lives. Every line of code we write, every model we train, every feature we build — it's all in service of that single truth.") }}</div>
      </div>
    </div>
  </div>
</section>
...
<section style="background:var(--off);">
  <div class="section-inner">
    <div class="section-tag">{{ __('Technology Stack') }}</div>
    <h2 class="section-title">{{ __("How it's built") }}</h2>
    <p class="section-sub">{{ __('A full-stack system combining a Laravel web application with a Python AI microservice — connected in real time.') }}</p>
    <div class="tech-grid">
      <div class="tech-card">
        <div class="tech-icon">🐘</div>
        <div class="tech-name">Laravel 12</div>
        <div class="tech-desc">{{ __('Web framework, auth, routing, DB') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🐍</div>
        <div class="tech-name">Python + FastAPI</div>
        <div class="tech-desc">{{ __('AI prediction microservice') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🤖</div>
        <div class="tech-name">scikit-learn</div>
        <div class="tech-desc">{{ $modelMetrics['model'] ?? 'Logistic Regression / k-NN' }} — {{ __('evaluated head-to-head') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🗄️</div>
        <div class="tech-name">MySQL</div>
        <div class="tech-desc">{{ __('Donor & hospital data storage') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🎨</div>
        <div class="tech-name">HTML / CSS / JS</div>
        <div class="tech-desc">{{ __('Responsive frontend UI') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🔐</div>
        <div class="tech-name">Laravel Guards</div>
        <div class="tech-desc">{{ __('Multi-role auth (Donor/Hospital/Admin)') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">📊</div>
        <div class="tech-name">pandas + numpy</div>
        <div class="tech-desc">{{ __('Data preprocessing & feature engineering') }}</div>
      </div>
      <div class="tech-card">
        <div class="tech-icon">🚀</div>
        <div class="tech-name">Uvicorn</div>
        <div class="tech-desc">{{ __('ASGI server for the AI API') }}</div>
      </div>
    </div>
  </div>
</section>
...
<section>
  <div class="section-inner">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:5rem;align-items:start;">
      <div>
        <div class="section-tag">{{ __('Project Journey') }}</div>
        <h2 class="section-title">{{ __('How LifeLink came to be') }}</h2>
        <p class="section-sub">{{ __('From a research idea to a working full-stack AI system.') }}</p>
      </div>
      <div class="timeline">
        <div class="tl-item">
          <div class="tl-dot"></div>
          <div class="tl-year">{{ __('Phase 1') }}</div>
          <div class="tl-title">{{ __('Research & Dataset Collection') }}</div>
          <div class="tl-text">{{ __('Collected and cleaned 10,000 donor records. Analysed eligibility criteria from the National Blood Transfusion Service of Sri Lanka.') }}</div>
        </div>
        <div class="tl-item">
          <div class="tl-dot"></div>
          <div class="tl-year">{{ __('Phase 2') }}</div>
          <div class="tl-title">{{ __('Model Training & Evaluation') }}</div>
          <div class="tl-text">{{ __('Compared Logistic Regression and k-NN classifiers on a held-out test set with precision, recall, and F1 scoring — k-NN won and is the model in production.') }}</div>
        </div>
        <div class="tl-item">
          <div class="tl-dot"></div>
          <div class="tl-year">{{ __('Phase 3') }}</div>
          <div class="tl-title">{{ __('FastAPI Microservice') }}</div>
          <div class="tl-text">{{ __('Wrapped the trained model in a FastAPI endpoint. Real-time prediction in under 200ms with graceful fallback if the AI service is unavailable.') }}</div>
        </div>
        <div class="tl-item">
          <div class="tl-dot"></div>
          <div class="tl-year">{{ __('Phase 4') }}</div>
          <div class="tl-title">{{ __('Laravel Full-Stack Integration') }}</div>
          <div class="tl-text">{{ __('Built the complete web platform — multi-role authentication, donor registration, hospital dashboard, admin panel, and live AI integration.') }}</div>
        </div>
        <div class="tl-item">
          <div class="tl-dot"></div>
          <div class="tl-year">{{ __('Now') }}</div>
          <div class="tl-title">{{ __('Live & Expanding') }}</div>
          <div class="tl-text">{{ __('LifeLink is live and being extended with SMS notifications, hospital blood stock management, and real-time donor-hospital matching.') }}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="footer-inner">
    <div class="footer-grid">
      <div class="footer-brand">
        ...
        <p>{{ __("Sri Lanka's AI-powered blood donor management system. Connecting donors and hospitals since 2025.") }}</p>
      </div>
      <div class="footer-col">
        <h4>{{ __('Platform') }}</h4>
        <a href="/">{{ __('Home') }}</a>
        <a href="/about">{{ __('About') }}</a>
        <a href="/find-donors">{{ __('Find Donors') }}</a>
        <a href="/contact">{{ __('Contact') }}</a>
      </div>
      <div class="footer-col">
        <h4>{{ __('Donors') }}</h4>
        <a href="{{ route('donor.register') }}">{{ __('Register') }}</a>
        <a href="{{ route('donor.login') }}">{{ __('Sign In') }}</a>
        <a href="/about">{{ __('How it Works') }}</a>
      </div>
      <div class="footer-col">
        <h4>{{ __('Legal') }}</h4>
    ...
    <div class="footer-bottom">
      <div class="footer-copy">© 2025 LifeLink. {{ __('Built for the National Blood Transfusion Service of Sri Lanka.') }}</div>
