...
<div class="sidebar">
  ...
  <div class="sb-nav">
    ...
    <a href="{{ route('donor.dashboard') }}#ai-health" class="sb-item">
      ...
      <span class="sb-tip">{{ __('Eligibility') }}</span>
    </a>
...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      <div class="greet">
        <div class="greet-sub">{{ __('Welcome back') }}</div>
    ...
    <div class="tb-right">
      ...
      <div class="notif-wrap">
        ...
        <div class="notif-dropdown" id="notifDropdown">
          ...
          @endforelse
          <div class="notif-dd-foot"><a href="{{ route('donor.notifications.index') }}">{{ __('View all →') }}</a></div>
        </div>
      </div>
      <div class="donor-pill">
        ...
        <span class="dp-name">{{ $donor->blood_group ?? 'N/A' }} {{ __('Donor') }}</span>
      </div>
  ...
  <div class="content">
    ...
    <div class="elig-banner {{ $eligible ? 'eb-eligible' : 'eb-not' }}">
      ...
      <div class="eb-text">
        <div class="eb-status">{{ __('AI Eligibility Status') }}</div>
        <div class="eb-title">
          {{ $eligible ? __('You are Eligible to Donate') : __('Currently Not Eligible') }}
        </div>
        <div class="eb-sub">
  {{ $modelMetrics['model'] ?? 'AI' }} {{ __('model') }} ·
  @if($donor->last_ai_check)
    {{ __('Last AI check:') }} {{ \Carbon\Carbon::parse($donor->last_ai_check)->diffForHumans() }}
  @else
    {{ __('AI check: at registration') }}
  @endif
</div>
        <div class="eb-date">
          <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          @if($eligible)
            {{ __('Next eligible donation:') }} <strong style="color:white;margin-left:4px;">
              {{ $daysUntil === 0 ? __('Available Now') : __('In :days days', ['days' => $daysUntil]) }}
            </strong>
          @else
            {{ __('Reason: Age, weight or hemoglobin below required threshold') }}
          @endif
        </div>
      </div>
      <div class="eb-right">
...
<div class="eb-score-label">{{ __('AI Confidence') }}</div>
      </div>
    </div>
    ...
    <div class="stat-row">
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-badge sb-g">{{ __('All time') }}</span>
        </div>
        <div class="sc-num">{{ $donations }}</div>
        <div class="sc-label">{{ __('Total Donations') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-badge sb-g">{{ __('Lives saved') }}</span>
        </div>
        <div class="sc-num">{{ $livesImpact }}</div>
        <div class="sc-label">{{ __('Lives Impacted') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-badge {{ $daysUntil === 0 ? 'sb-g' : 'sb-a' }}">
            {{ $daysUntil === 0 ? __('Ready now') : __('Cooldown') }}
          </span>
        </div>
        <div class="sc-num">{{ $daysUntil }}</div>
        <div class="sc-label">{{ __('Days Until Available') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
    ...
    <div class="grid-main">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card" id="ai-health" style="scroll-margin-top:74px;">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('AI Health Breakdown') }}</div>
              <div class="card-s">{{ __('Factors used in your eligibility prediction') }}</div>
            </div>
            <span class="sc-badge {{ $eligible ? 'sb-g' : 'sb-r' }}" style="font-size:0.68rem;padding:3px 10px;border-radius:20px;">
              {{ $eligible ? __('Eligible') : __('Not Eligible') }}
            </span>
          </div>
          ...
          <div class="ai-breakdown">
            <div class="ai-metric">
              ...
              <div class="am-label">{{ __('Age (years)') }}</div>
              ...
              <div class="am-status {{ $age >= 18 ? 'st-ok' : 'st-warn' }}">
                {{ $age >= 18 ? '✓ '.__('Meets criteria (18+)') : '✗ '.__('Must be 18+') }}
              </div>
            </div>
            <div class="ai-metric">
              ...
              <div class="am-label">{{ __('Weight') }}</div>
              ...
              <div class="am-status {{ $weight >= 50 ? 'st-ok' : 'st-warn' }}">
                {{ $weight >= 50 ? '✓ '.__('Meets criteria (50+)') : '✗ '.__('Must be 50+ kg') }}
              </div>
            </div>
            <div class="ai-metric">
              <div class="am-val">{{ $hemo > 0 ? number_format($hemo,1) : '—' }}</div>
              <div class="am-label">{{ __('Hemoglobin (g/dL)') }}</div>
              ...
              <div class="am-status {{ $hemo >= 12 ? 'st-ok' : 'st-warn' }}">
                {{ $hemo >= 12 ? '✓ '.__('Meets criteria (12+)') : '✗ '.__('Must be 12+ g/dL') }}
              </div>
            </div>
          </div>
          ...
          <div class="cooldown-wrap">
            <div class="cd-top">
              <span class="cd-label">{{ __('56-Day Recovery Period') }}</span>
              <span class="cd-days" style="color:{{ $daysUntil === 0 ? 'var(--green)' : 'var(--amber)' }}">
                {{ $daysUntil === 0 ? '✓ '.__('Completed — Available now') : $daysUntil.' '.__('days remaining') }}
              </span>
            </div>
            <div class="cd-track">
            ...
            <div class="cd-marks">
              <span>{{ __('Last donation:') }} {{ $lastDonation ? \Carbon\Carbon::parse($lastDonation)->format('d M Y') : __('Never') }}</span>
              <span>{{ __('Day 28') }}</span>
              <span>{{ __('Day 56') }} {{ $daysUntil === 0 ? '✓' : '' }}</span>
            </div>
          </div>
          ...
          <div style="display:flex;gap:10px;">
            <div style="flex:1;background:{{ $eligible ? 'var(--green-bg)' : 'var(--warning-light)' }};border:1px solid {{ $eligible ? 'var(--green-b)' : 'var(--border)' }};border-radius:9px;padding:0.85rem 1rem;display:flex;align-items:center;gap:8px;">
              ...
              <div>
                <div style="font-size:0.78rem;font-weight:500;color:{{ $eligible ? 'var(--green)' : 'var(--warning)' }};">
                  {{ $eligible ? __('All criteria met') : __('Criteria not met') }}
                </div>
                <div style="font-size:0.7rem;color:{{ $eligible ? '#166534' : 'var(--warning-dark)' }};">
                  {{ $eligible ? __('Ready to donate today') : __('Update your health profile') }}
                </div>
              </div>
            </div>
            <div style="flex:1;background:var(--blue-bg);border:1px solid var(--blue-b);border-radius:9px;padding:0.85rem 1rem;display:flex;align-items:center;gap:8px;">
              ...
              <div>
                <div style="font-size:0.78rem;font-weight:500;color:var(--blue);">{{ __('Model:') }} {{ $modelMetrics['model'] ?? 'unavailable' }}</div>
                <div style="font-size:0.7rem;color:#1E40AF;">
                  @if(!empty($modelMetrics['accuracy']))
                    {{ number_format($modelMetrics['accuracy'], 2) }}% {{ __('system accuracy') }}
                  @else
                    {{ __('Accuracy not available') }}
                  @endif
                </div>
              </div>
            </div>
        ...
        <div class="card" id="appointments" style="scroll-margin-top:74px;">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('Upcoming Appointment') }}</div>
              <div class="card-s">@if($upcomingAppointment) {{ $upcomingAppointment->statusLabel() }} @else {{ __('No appointment booked') }} @endif</div>
            </div>
            <a href="{{ route('donor.appointments.index') }}" class="card-action">{{ __('Manage appointments →') }}</a>
          </div>
            ...
            <div class="hist-list">
              <div class="hist-item">
                ...
                <div class="hist-info">
                  ...
                  <div class="hist-s">{{ $upcomingAppointment->bloodRequest?->blood_group }} {{ __('donation') }}</div>
                </div>
          ...
          @else
            <div class="empty-state">
              {{ __('No upcoming appointment. Respond "Available" to a blood request, then book an appointment from the Appointments page.') }}
            </div>
          @endif
        </div>
        ...
        <div class="card" id="donation-history" style="scroll-margin-top:74px;">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('Donation History') }}</div>
              <div class="card-s">{{ $donations }} {{ __('completed donation') }}{{ $donations !== 1 ? 's' : '' }}</div>
            </div>
          </div>
          @if($donor->donations->count() > 0)
            <div class="hist-list">
              ...
              <div class="hist-item">
                ...
                <div class="hist-info">
                  <div class="hist-t">{{ __('Whole Blood Donation') }}</div>
                ...
                <div class="hist-right">
                  ...
                  <span class="badge b-ok">{{ __('Completed') }}</span>
                </div>
              </div>
              @endforeach
            </div>
          @else
            <div class="empty-state">
              {{ __('No recorded donations yet. Once you donate, your care team will log it here.') }}
            </div>
          @endif
        </div>
      ...
      <div class="right-col">
        ...
        <div class="profile-card">
          <div class="pc-top">
            ...
            <div><span class="pc-blood">{{ $donor->blood_group ?? 'N/A' }} {{ __('Donor') }}</span></div>
          </div>
          <div class="pc-body">
            <div class="pc-row">
              <span class="pc-key">{{ __('Age') }}</span>
              <span class="pc-val">{{ $age > 0 ? $age.' '.__('years') : __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Weight') }}</span>
              <span class="pc-val">{{ $weight > 0 ? $weight.' kg' : __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Hemoglobin') }}</span>
              <span class="pc-val">{{ $hemo > 0 ? number_format($hemo,1).' g/dL' : __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Gender') }}</span>
              <span class="pc-val">{{ $donor->gender ?? __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('City') }}</span>
              <span class="pc-val">{{ $donor->city ?? __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('District') }}</span>
              <span class="pc-val">{{ $donor->district ?? __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Email') }}</span>
              <span class="pc-val" style="font-size:0.72rem;">{{ $donor->email }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Phone') }}</span>
              <span class="pc-val">{{ $donor->phone ?? __('Not set') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Donations') }}</span>
              <span class="pc-val">{{ $donations }} {{ __('completed') }}</span>
            </div>
            <div class="pc-row">
              <span class="pc-key">{{ __('Member since') }}</span>
              <span class="pc-val">{{ $donorSince }}</span>
            </div>
            <a href="{{ route('donor.profile.edit') }}" class="edit-btn" style="display:block;text-align:center;text-decoration:none;">{{ __('Edit Profile') }}</a>
          </div>
        </div>
        ...
        <div class="card" id="notifications" style="scroll-margin-top:74px;">
          <div class="card-hd">
            <div><div class="card-t">{{ __('Notifications') }}</div></div>
            <a href="{{ route('donor.notifications.index') }}" class="card-action">{{ __('View all →') }}</a>
          </div>
          <div class="notif-list">
            ...
            @empty
              <div class="notif-item" style="opacity:0.65;">
                ...
                <div>
                  <div class="notif-text">{{ __('Welcome to') }} <strong>LifeLink</strong>! {{ __('Your donor profile has been created successfully.') }}</div>
    ...
    <div style="margin:1.75rem 0 0.75rem;">
      ...
      <div style="font-size:0.72rem;color:var(--muted);margin-top:2px;">{{ __('Your personal donation and AI prediction history') }}</div>
    </div>

    <div class="card" style="margin-bottom:14px;">
      <div class="card-hd"><div class="card-t">{{ __('Personal Donation History') }}</div></div>
      <div style="height:200px;"><canvas id="chartDonationHistory"></canvas></div>
    </div>

    <div class="card" style="margin-bottom:14px;">
      <div class="card-hd"><div class="card-t">{{ __('Donation Timeline') }}</div></div>
      @if($donationTimeline->isEmpty())
        <div style="font-size:0.82rem;color:var(--muted);">{{ __('No donations recorded yet — your timeline will appear here once you donate.') }}</div>
      @else
        <div style="position:relative;padding-left:22px;">
          <div style="position:absolute;left:6px;top:4px;bottom:4px;width:2px;background:var(--gray-b);"></div>
          @foreach($donationTimeline as $d)
            <div style="position:relative;padding-bottom:1.1rem;">
              <div style="position:absolute;left:-22px;top:2px;width:12px;height:12px;border-radius:50%;background:var(--secondary);border:2px solid white;box-shadow:0 0 0 1px var(--secondary-b, var(--secondary));"></div>
              <div style="font-size:0.85rem;font-weight:500;color:var(--text);">{{ $d->donation_date->format('d M Y') }}</div>
              <div style="font-size:0.78rem;color:var(--muted);">{{ $d->blood_group ?? $donor->blood_group ?? 'N/A' }} · {{ $d->donation_center ?? 'NBTS' }} · {{ $d->units }} {{ __('unit') }}{{ $d->units > 1 ? 's' : '' }}</div>
            </div>
          @endforeach
        </div>
      @endif
    ...
    <div class="card" style="margin-bottom:14px;">
      <div class="card-hd">
        <div>
          <div class="card-t">{{ __('AI Prediction History') }}</div>
          <div class="card-s">{{ __('Confidence score over time, across every AI check on your profile') }}</div>
        </div>
      </div>
      @if($aiPredictionHistory->isEmpty())
        <div style="font-size:0.82rem;color:var(--muted);">{{ __('No AI predictions logged yet.') }}</div>
      @else
        <div style="height:200px;"><canvas id="chartAiHistory"></canvas></div>
      @endif
    </div>
...
(function () {
  ...
  var donationHistory = @json($donationHistoryChart);
  new Chart(document.getElementById('chartDonationHistory'), {
    type: 'bar',
    data: {
      labels: donationHistory.map(function (r) { return r.label; }),
      datasets: [{ label: 'Donations', data: donationHistory.map(function (r) { return r.count; }), backgroundColor: THEME.secondary, borderRadius: 4 }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { beginAtZero: true, ticks: { precision: 0 }, grid: { color: THEME.grid } } } }
  });

  var aiHistory = @json($aiPredictionHistory);
  if (aiHistory.length) {
    new Chart(document.getElementById('chartAiHistory'), {
      type: 'line',
      data: {
        labels: aiHistory.map(function (r) { return r.label; }),
        datasets: [{
          label: 'Confidence',
          data: aiHistory.map(function (r) { return r.confidence; }),
