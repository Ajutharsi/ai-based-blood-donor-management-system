...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      ...
      <div class="tb-title">{{ __('AI Matched Donors') }}</div>
    </div>
    <div class="tb-right">
      <div class="hosp-pill">
  ...
  <div class="content">
    ...
    <div class="req-banner {{ $bannerClass }}">
      ...
      <div class="rb-text">
        <div class="rb-label">{{ __('Blood Request Submitted · AI Matching Complete') }}</div>
        <div class="rb-title">{{ $bloodRequest->blood_group }} {{ __('Blood Required') }}</div>
        <div class="rb-sub">{{ $bloodRequest->units_needed }} unit{{ $bloodRequest->units_needed > 1 ? 's' : '' }} {{ __('needed') }} · {{ $hospital->name }}</div>
        <div class="rb-meta">
          <span class="rb-tag">{{ ucfirst($bloodRequest->urgency) }}</span>
          <span class="rb-tag">{{ $bloodRequest->ward ?? 'General' }}</span>
          <span class="rb-tag">{{ __('Request') }} #{{ $bloodRequest->id }}</span>
          @if($bloodRequest->required_by)
            <span class="rb-tag">{{ __('Due:') }} {{ \Carbon\Carbon::parse($bloodRequest->required_by)->format('d M Y') }}</span>
          @endif
        </div>
      </div>
      <div class="rb-right">
        <div class="rb-count">{{ $matchCount }}</div>
        <div class="rb-count-label">{{ __('Donors Found') }}</div>
      </div>
    </div>
    ...
    <div class="main-grid">
      ...
      <div class="card">
        <div class="ai-header">
          ...
          <div>
            <div class="ai-hd-t">{{ __('AI Matched Donors') }}</div>
            <div class="ai-hd-s">{{ __('Ranked by match score — blood compatibility · district · AI signals ·') }} {{ ucfirst($bloodRequest->urgency) }} {{ __('urgency') }}</div>
          </div>
          <span class="ai-status">{{ $matchCount }} {{ __('eligible found') }}</span>
        </div>

        @if($matched_donors->isNotEmpty())
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:1rem;">
            <span style="font-size:0.72rem;color:var(--muted);">{{ __('Sort by:') }}</span>
            @php $sortLinks = ['match_score' => __('Match Score'), 'distance' => __('Distance'), 'confidence' => __('AI Confidence')]; @endphp
        ...
        @if($matched_donors->isEmpty())
          <div class="empty-state">
            ...
            <h3>{{ __('No Eligible Donors Found') }}</h3>
            <p>{{ __('There are currently no eligible donors with blood group') }} {{ $bloodRequest->blood_group }}.<br>
            {{ __('The system will notify you when a matching donor registers.') }}</p>
          </div>
        @else
          <div class="match-list">
            @foreach($matched_donors as $index => $donor)
              ...
              <div class="match-card {{ $index === 0 ? 'top' : '' }}">
                @if($index === 0)
                  <div class="top-badge">{{ __('Best Match') }}</div>
                @endif
                ...
                <div class="mc-score-wrap">
                  <span class="mc-score-label" style="font-weight:600;color:var(--accent-dark);">{{ __('Match') }}</span>
                ...
                {{-- AI SCORE BAR --}}
                <div class="mc-score-wrap">
                  <span class="mc-score-label">{{ __('AI score') }}</span>
                  <div class="mc-score-bar">
...
@if($donor->response_probability > 0)
  ...
  @endphp
  <div class="mc-score-wrap">
    <span class="mc-score-label" style="color:{{ $respColor }}">{{ __('Response') }}</span>
    <div class="mc-score-bar">
                ...
                <div class="mc-tags">
                  ...
                  @if($donorResponse)
                    @if($donorResponse->status === 'available')
                      <span class="mc-tag tag-g">✓ {{ __('Confirmed available') }}</span>
                    @else
                      <span class="mc-tag tag-gray">{{ __('Responded: not available') }}</span>
                    @endif
                  @endif
                  <span class="mc-tag tag-g">{{ __('Eligible') }}</span>
                  @if($exactMatch)
                    <span class="mc-tag tag-g">{{ __('Exact') }} {{ $donor->blood_group }} {{ __('match') }}</span>
                  @else
                    <span class="mc-tag tag-b">{{ __('Compatible donor') }}</span>
                  @endif
                  @if($sameDistrict)
                    <span class="mc-tag tag-g">{{ __('Same district') }}</span>
                  @elseif($donor->district)
                    <span class="mc-tag tag-gray">{{ $donor->district }}</span>
                  @endif
                  @if($donor->distance_km !== null)
                    <span class="mc-tag tag-b">{{ number_format($donor->distance_km, 1) }} km · {{ __($donor->travel_category) }}</span>
                  @else
                    <span class="mc-tag tag-gray">{{ __('Distance unknown') }}</span>
                  @endif
                  @if($donor->hemoglobin)
                    <span class="mc-tag tag-b">{{ __('Hemo') }} {{ number_format($donor->hemoglobin,1) }}</span>
                  @endif
                  @if($donor->weight_kg)
                    <span class="mc-tag tag-b">{{ $donor->weight_kg }} kg</span>
                  @endif
                  @if($donations >= 5)
                    <span class="mc-tag tag-g">{{ __('Experienced') }}</span>
                  @elseif($donations === 0)
                    <span class="mc-tag tag-gray">{{ __('First time') }}</span>
                  @endif
                </div>
      ...
      <div class="right-col">
        ...
        <div class="req-detail-card">
          <div class="rd-top">
            ...
            <div class="rd-blood-label">{{ __('Blood Group') }}</div>
          </div>
          <div class="rd-body">
            <div class="rd-row">
              <span class="rd-key">{{ __('Units Needed') }}</span>
              <span class="rd-val">{{ $bloodRequest->units_needed }}</span>
            </div>
            <div class="rd-row">
              <span class="rd-key">{{ __('Urgency') }}</span>
            ...
            <div class="rd-row">
              <span class="rd-key">{{ __('Ward') }}</span>
              <span class="rd-val">{{ $bloodRequest->ward ?? 'General' }}</span>
            </div>
            <div class="rd-row">
              <span class="rd-key">{{ __('Required By') }}</span>
              <span class="rd-val">
                {{ $bloodRequest->required_by ? \Carbon\Carbon::parse($bloodRequest->required_by)->format('d M Y') : 'ASAP' }}
              </span>
            </div>
            <div class="rd-row">
              <span class="rd-key">{{ __('Status') }}</span>
              <span class="badge b-pen">{{ __('Pending') }}</span>
            </div>
            <div class="rd-row">
              <span class="rd-key">{{ __('Request ID') }}</span>
        ...
        <div class="card">
          <div class="card-hd">
            <div class="card-t">{{ __('How to Contact Donors') }}</div>
          </div>
          <div class="tip-item">
            <div class="tip-dot" style="background:var(--secondary);"></div>
            <div class="tip-text">{{ __('Click') }} <strong>{{ __('Notify') }}</strong> {{ __('to email the donor about your blood request, with a link for them to confirm their availability.') }}</div>
          </div>
          <div class="tip-item">
            <div class="tip-dot" style="background:var(--blue);"></div>
            <div class="tip-text">{{ __('Click') }} <strong>{{ __('Call') }}</strong> {{ __("to directly call the donor's registered phone number.") }}</div>
          </div>
          <div class="tip-item">
            <div class="tip-dot" style="background:var(--green);"></div>
            <div class="tip-text">{{ __('Once a donor confirms, mark the request as') }} <strong>{{ __('Fulfilled') }}</strong> {{ __('to close it.') }}</div>
          </div>
          <div class="tip-item">
            <div class="tip-dot" style="background:var(--amber);"></div>
            <div class="tip-text">{{ __('Donors are ranked by') }} <strong>{{ __('match score') }}</strong> {{ __('— blood compatibility, district, and AI-predicted reliability — contact the top match first.') }}</div>
          </div>
        </div>
