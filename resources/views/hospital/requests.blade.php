...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      ...
      <div class="tb-title">{{ __('Blood Request History') }}</div>
    </div>
    <div class="tb-right">
      <div class="hosp-pill">
  ...
  <div class="content">
    ...
    <div class="page-hd">
      <div class="page-hd-left">
        <div class="page-label">{{ __('History') }}</div>
        <h1>{{ __('All Blood Requests') }}</h1>
      </div>
    ...
    <div class="stat-row">
      <div class="sc">
        <div class="sc-top"><div class="sc-icon ic-r"><svg viewBox="0 0 24 24"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg></div><span class="sc-trend t-r">{{ __('All time') }}</span></div>
        <div class="sc-num">{{ $total }}</div><div class="sc-label">{{ __('Total Requests') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top"><div class="sc-icon ic-b"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div><span class="sc-trend t-b">{{ __('Active') }}</span></div>
        <div class="sc-num">{{ $pending }}</div><div class="sc-label">{{ __('Pending') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top"><div class="sc-icon ic-g"><svg viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div><span class="sc-trend t-g">{{ __('Done') }}</span></div>
        <div class="sc-num">{{ $fulfilled }}</div><div class="sc-label">{{ __('Fulfilled') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top"><div class="sc-icon ic-a"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><span class="sc-trend t-a">{{ __('Month') }}</span></div>
        <div class="sc-num">{{ $thisMonth }}</div><div class="sc-label">{{ __('This Month') }}</div>
      </div>
    </div>
    ...
    <form method="GET" action="{{ route('hospital.requests.index') }}">
      <div class="filter-bar">
        ...
        <select name="urgency" class="filter-select">
          <option value="">{{ __('All Urgency') }}</option>
          <option value="standard"  {{ request('urgency') === 'standard'  ? 'selected' : '' }}>{{ __('Standard') }}</option>
          <option value="urgent"    {{ request('urgency') === 'urgent'    ? 'selected' : '' }}>{{ __('Urgent') }}</option>
          <option value="critical"  {{ request('urgency') === 'critical'  ? 'selected' : '' }}>{{ __('Critical') }}</option>
        </select>
        ...
        <a href="{{ route('hospital.requests.index') }}" class="btn-reset">{{ __('Reset') }}</a>
      </div>
    </form>
    ...
    <div class="filter-tabs">
      <a href="{{ route('hospital.requests.index') }}"
         class="ftab {{ !request('status') ? 'active' : '' }}">{{ __('All') }} ({{ $total }})</a>
      <a href="{{ route('hospital.requests.index', ['status' => 'pending']) }}"
         class="ftab {{ request('status') === 'pending' ? 'active' : '' }}">{{ __('Pending') }} ({{ $pending }})</a>
      <a href="{{ route('hospital.requests.index', ['status' => 'fulfilled']) }}"
         class="ftab {{ request('status') === 'fulfilled' ? 'active' : '' }}">{{ __('Fulfilled') }} ({{ $fulfilled }})</a>
    </div>
    ...
    <div class="table-card">
      ...
      @if($requests->hasPages())
        <div class="pagination-wrap">
          <div class="page-info">
            {{ __('Showing') }} {{ $requests->firstItem() }}–{{ $requests->lastItem() }} {{ __('of') }} {{ $requests->total() }} {{ __('requests') }}
          </div>
