<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .card-act{font-size:0.75rem;color:var(--primary);text-decoration:none;cursor:pointer;}
  ...
  .bb:hover{border-color:var(--primary);color:var(--primary);background:var(--primary-light);}
  .bb.sel{border-color:var(--primary);color:var(--primary);background:var(--primary-light);}
  ...
  .ub.sel-crit{border-color:var(--amber-dark);color:var(--amber-dark);background:var(--amber-b);}
  ...
  .field input:focus,.field select:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(29,78,216,0.07);}
  .field textarea{min-height:70px;resize:none;}
  .error-msg{font-size:0.72rem;color:var(--amber-dark);margin-top:3px;display:block;}
  .btn-req{width:100%;padding:0.8rem;border:none;border-radius:9px;background:var(--primary);color:white;font-family:'DM Sans',sans-serif;font-size:0.9rem;font-weight:500;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:0.5rem;}
  .btn-req:hover{background:var(--primary-dark);transform:translateY(-1px);}
  .btn-req svg{width:16px;height:16px;stroke:white;fill:none;stroke-width:2;}

  /* REQUEST HISTORY */
  .rh-list{display:flex;flex-direction:column;gap:0;}
  .rh-item{display:flex;align-items:center;gap:12px;padding:0.85rem 0;border-bottom:1px solid rgba(29,78,216,0.06);}
  .rh-item:last-child{border-bottom:none;}
  .rh-blood{width:40px;height:40px;border-radius:9px;background:var(--primary-light);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:0.875rem;color:var(--primary-dark);flex-shrink:0;}
  .rh-info{flex:1;}
  ...
  .hi-row{display:flex;align-items:center;justify-content:space-between;padding:0.5rem 0;border-bottom:1px solid rgba(29,78,216,0.06);}
...
<div class="sidebar">
  ...
  <div class="sb-nav">
    ...
    <a href="{{ route('hospital.reports.index') }}" class="sb-item">
...
<div class="main">
      ...
      <div class="notif-wrap">
        ...
        <div class="notif-dropdown" id="notifDropdown">
          <div class="notif-dd-head">
            ...
            @if($unreadNotifications > 0)
              <form method="POST" action="{{ route('hospital.notifications.readAll') }}" style="margin:0;">
                @csrf
                <button type="submit" class="notif-dd-markall">{{ __('Mark all read') }}</button>
              </form>
            @endif
          </div>
          ...
          @empty
            <div class="notif-dd-empty">{{ __('No notifications yet.') }}</div>
          @endforelse
          <div class="notif-dd-foot"><a href="{{ route('hospital.notifications.index') }}">{{ __('View all →') }}</a></div>
        </div>
      </div>
      <div class="hosp-pill">
        <div class="hosp-av">
  ...
  <div class="content">
    ...
    <div class="stat-row">
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-trend t-r">{{ __('Total') }}</span>
        </div>
        <div class="sc-num">{{ $stats['total_requests'] }}</div>
        <div class="sc-label">{{ __('Blood Requests') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-trend t-b">{{ __('Active') }}</span>
        </div>
        <div class="sc-num">{{ $stats['pending'] }}</div>
        <div class="sc-label">{{ __('Pending Requests') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-trend t-g">{{ __('Done') }}</span>
        </div>
        <div class="sc-num">{{ $stats['fulfilled'] }}</div>
        <div class="sc-label">{{ __('Fulfilled') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top">
          ...
          <span class="sc-trend t-a">{{ __('Month') }}</span>
        </div>
        <div class="sc-num">{{ $stats['this_month'] }}</div>
        <div class="sc-label">{{ __('This Month') }}</div>
      </div>
    </div>
    ...
    <div class="main-grid">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('New Blood Request') }}</div>
              <div class="card-s">{{ __('AI will instantly find matched eligible donors') }}</div>
            </div>
          </div>
          ...
          <form method="POST" action="{{ route('hospital.request.store') }}">
            ...
            {{-- BLOOD GROUP --}}
            <span class="form-label">{{ __('Blood Group Required') }} <span>*</span></span>
            ...
            {{-- URGENCY --}}
            <span class="form-label">{{ __('Urgency Level') }} <span>*</span></span>
            ...
            <div class="urg-row">
              <button type="button" class="ub {{ old('urgency','standard') === 'standard' ? 'sel-std' : '' }}"
                ...
                <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{ __('Standard') }}
              </button>
              <button type="button" class="ub {{ old('urgency') === 'urgent' ? 'sel-urg' : '' }}"
                      onclick="selectUrgency(this,'urgent','sel-urg')">
                <svg viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>{{ __('Urgent') }}
              </button>
              <button type="button" class="ub {{ old('urgency') === 'critical' ? 'sel-crit' : '' }}"
                      onclick="selectUrgency(this,'critical','sel-crit')">
                <svg viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>{{ __('Critical') }}
              </button>
            </div>
            ...
            <div class="form-row">
              <div class="field">
                <label>{{ __('Units Needed') }} <span style="color:var(--amber)">*</span></label>
                <input type="number" name="units_needed"
                       value="{{ old('units_needed', 1) }}"
                       min="1" max="20" placeholder="{{ __('e.g. 2') }}">
                @error('units_needed')<span class="error-msg">{{ $message }}</span>@enderror
              </div>
              <div class="field">
                <label>{{ __('Ward / Department') }}</label>
                <select name="ward">
                  @foreach(['' => 'Select ward','ICU' => 'ICU','Surgery' => 'Surgery','Maternity' => 'Maternity','Trauma' => 'Trauma','Paediatric' => 'Paediatric','Oncology' => 'Oncology','General' => 'General'] as $val => $label)
                    <option value="{{ $val }}" {{ old('ward') === $val ? 'selected' : '' }}>{{ __($label) }}</option>
                  @endforeach
                </select>
              </div>
              <div class="field">
                <label>{{ __('Required By') }}</label>
            ...
            <div class="field" style="margin-bottom:1.1rem;">
              <label>{{ __('Special Notes') }}</label>
              <textarea name="notes" placeholder="{{ __('Patient condition, special requirements...') }}">{{ old('notes') }}</textarea>
            </div>

            <button type="submit" class="btn-req">
              <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>
              {{ __('Find Matched Donors with AI') }}
            </button>
          </form>
        </div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __('Recent Requests') }}</div>
              <div class="card-s">{{ __('Your latest blood requests') }}</div>
            </div>
            <a href="{{ route('hospital.requests.index') }}" class="card-act">{{ __('View all →') }}</a>
          </div>
          <div class="rh-list">
            ...
            @empty
              <div class="empty-state">{{ __('No requests yet. Submit your first blood request above.') }}</div>
            @endforelse
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-hd">
            <div>
              <div class="card-t">{{ __("Today's Appointments") }}</div>
              <div class="card-s">{{ $pendingAppointmentsCount }} {{ __('awaiting approval') }}</div>
            </div>
            <a href="{{ route('hospital.appointments.index') }}" class="card-act">{{ __('Manage appointments →') }}</a>
          </div>
          <div class="rh-list">
            @forelse($todayAppointments as $a)
              <div class="rh-item">
                <div class="rh-blood">{{ $a->bloodRequest?->blood_group }}</div>
                <div class="rh-info">
                  <div class="rh-t">{{ $a->donor?->full_name ?? 'Donor' }}</div>
                  <div class="rh-s">{{ $a->appointment_time }} · {{ ucfirst($a->status) }}</div>
                </div>
              </div>
            @empty
              <div class="empty-state">{{ __('No appointments scheduled for today.') }}</div>
            @endforelse
          </div>
          @if($upcomingAppointments->isNotEmpty())
            <div style="font-size:0.7rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em;margin:0.9rem 0 0.5rem;">{{ __('Upcoming') }}</div>
            <div class="rh-list">
              @foreach($upcomingAppointments as $a)
                <div class="rh-item">
                  ...
                  <div class="rh-info">
                    <div class="rh-t">{{ $a->donor?->full_name ?? 'Donor' }}</div>
                    <div class="rh-s">{{ $a->appointment_date->format('d M Y') }} · {{ $a->appointment_time }}</div>
                  </div>
                </div>
              @endforeach
            </div>
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="hosp-info-card">
          <div class="hi-top">
            ...
            <div style="margin-top:6px;">
              <span class="hi-tag">{{ $hospital->is_verified ? '✓ '.__('Verified') : __('Pending Verification') }}</span>
            </div>
          </div>
          <div class="hi-body">
            <div class="hi-row">
              <span class="hi-key">{{ __('Email') }}</span>
              <span class="hi-val" style="font-size:0.72rem;">{{ $hospital->email }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('Phone') }}</span>
              <span class="hi-val">{{ $hospital->phone ?? '—' }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('City') }}</span>
              <span class="hi-val">{{ $hospital->city ?? '—' }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('District') }}</span>
              <span class="hi-val">{{ $hospital->district ?? '—' }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('Address') }}</span>
              <span class="hi-val" style="font-size:0.72rem;text-align:right;max-width:160px;">{{ $hospital->address ?? '—' }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('Total Requests') }}</span>
              <span class="hi-val">{{ $stats['total_requests'] }}</span>
            </div>
            <div class="hi-row">
              <span class="hi-key">{{ __('Fulfilled') }}</span>
        ...
        <div class="card">
          <div class="card-hd">
            <div class="card-t">{{ __('Blood Group Stats') }}</div>
          </div>
          @php
          ...
          <div style="display:flex;flex-direction:column;gap:8px;">
            @foreach($bgCounts as $bg => $count)
              ...
              <div style="display:flex;align-items:center;gap:8px;">
                <span style="display:inline-block;background:var(--accent-light);color:var(--accent-dark);font-size:0.72rem;font-weight:600;padding:2px 7px;border-radius:4px;width:36px;text-align:center;flex-shrink:0;">{{ $bg }}</span>
                <div style="flex:1;background:var(--gray);border-radius:3px;height:5px;overflow:hidden;">
                  <div style="height:5px;border-radius:3px;background:var(--accent);width:{{ $pct }}%"></div>
                </div>
                <span style="font-size:0.72rem;font-weight:500;color:var(--text);min-width:20px;text-align:right;">{{ $count }}</span>
              </div>
            @endforeach
          </div>
          <div style="font-size:0.7rem;color:var(--muted);margin-top:0.75rem;">{{ __('Eligible donors available per blood group') }}</div>
        </div>

        {{-- NEARBY DONORS (real distance, requires this hospital's location) --}}
        <div class="card">
          <div class="card-hd">
            <div class="card-t">{{ __('Nearby Eligible Donors') }}</div>
          </div>
          @if(!$hospital->hasLocation())
            <div style="font-size:0.78rem;color:var(--muted);line-height:1.5;">
              {{ __("Set your hospital's location in") }} <a href="{{ route('hospital.profile.edit') }}" style="color:var(--primary);font-weight:500;">{{ __('Profile Settings') }}</a> {{ __('to see nearby donors ranked by real distance.') }}
            </div>
          @elseif($nearbyDonors->isEmpty())
            <div style="font-size:0.78rem;color:var(--muted);">{{ __('No eligible donors have set their location yet.') }}</div>
          @else
            <div style="display:flex;justify-content:space-between;margin-bottom:0.85rem;padding-bottom:0.75rem;border-bottom:1px solid var(--border);">
              <div>
                <div style="font-size:0.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em;">{{ __('Avg. Distance') }}</div>
                <div style="font-size:1.1rem;font-weight:700;color:var(--text);font-family:'Playfair Display',serif;">{{ $averageDonorDistance }} km</div>
              </div>
              <div style="text-align:right;">
                <div style="font-size:0.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em;">{{ __('Closest') }}</div>
                <div style="font-size:1.1rem;font-weight:700;color:var(--secondary);font-family:'Playfair Display',serif;">{{ $closestDonor->distance_km }} km</div>
              </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:8px;">
              @foreach($nearbyDonors as $donor)
                <div style="display:flex;align-items:center;justify-content:space-between;font-size:0.8rem;">
                  <span style="color:var(--text);">{{ $donor->full_name }} <span style="color:var(--muted);font-size:0.72rem;">({{ $donor->blood_group }})</span></span>
                  <span style="color:var(--primary);font-weight:500;">{{ $donor->distance_km }} km</span>
                </div>
              @endforeach
            </div>
          @endif
    ...
    {{-- ── ADVANCED ANALYTICS ── --}}
    <div style="margin:1.75rem 0 0.75rem;">
      <div style="font-size:0.78rem;font-weight:500;color:var(--text);">{{ __('Advanced Analytics') }}</div>
      <div style="font-size:0.72rem;color:var(--muted);margin-top:2px;">{{ __("Your hospital's trends over the last 6 months") }}</div>
    </div>

    <div class="main-grid" style="grid-template-columns:1fr 1fr;">
      <div class="card">
        <div class="card-hd"><div class="card-t">{{ __('Monthly Requests') }}</div></div>
        <div style="height:220px;"><canvas id="chartMonthlyRequests"></canvas></div>
      </div>
      <div class="card">
        <div class="card-hd"><div class="card-t">{{ __('Donation Statistics') }}</div></div>
        <div style="height:220px;"><canvas id="chartDonationStats"></canvas></div>
      </div>
    </div>

    <div class="main-grid" style="grid-template-columns:1fr 1fr;">
      <div class="card">
        <div class="card-hd"><div class="card-t">{{ __('Inventory Trends') }}</div></div>
        <div style="height:220px;"><canvas id="chartInventoryTrends"></canvas></div>
      </div>
      <div class="card">
        <div class="card-hd"><div class="card-t">{{ __('Appointment Trends') }}</div></div>
        <div style="height:220px;"><canvas id="chartAppointmentTrends"></canvas></div>
      </div>
    </div>
...
<script>
(function () {
  var THEME = { primary: '#1D4ED8', secondary: '#0D9488', accent: '#06B6D4', warning: '#D97706', green: '#16A34A', muted: '#64748B', grid: 'rgba(29,78,216,0.08)' };
  Chart.defaults.font.family = "'DM Sans', sans-serif";
  Chart.defaults.color = THEME.muted;
  var commonScales = { x: { grid: { display: false } }, y: { beginAtZero: true, grid: { color: THEME.grid } } };

  var monthlyRequests = @json($monthlyRequests);
  new Chart(document.getElementById('chartMonthlyRequests'), {
    type: 'line',
    data: {
      labels: monthlyRequests.map(function (r) { return r.label; }),
      datasets: [{ label: 'Requests', data: monthlyRequests.map(function (r) { return r.total; }), borderColor: THEME.primary, backgroundColor: 'rgba(29,78,216,0.12)', fill: true, tension: 0.35 }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: commonScales }
  });

  var donationStats = @json($donationStatistics);
  new Chart(document.getElementById('chartDonationStats'), {
    type: 'bar',
    data: {
      labels: donationStats.map(function (r) { return r.label; }),
      datasets: [{ label: 'Donations', data: donationStats.map(function (r) { return r.count; }), backgroundColor: THEME.secondary, borderRadius: 4 }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: commonScales }
  });

  var invTrends = @json($inventoryTrends);
  new Chart(document.getElementById('chartInventoryTrends'), {
    type: 'bar',
    data: {
      labels: invTrends.map(function (r) { return r.label; }),
      datasets: [
        { label: 'Units Added', data: invTrends.map(function (r) { return r.added; }), backgroundColor: THEME.secondary, borderRadius: 4 },
        { label: 'Units Removed', data: invTrends.map(function (r) { return r.removed; }), backgroundColor: THEME.warning, borderRadius: 4 }
      ]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: commonScales }
  });

  var apptTrends = @json($appointmentTrends);
  new Chart(document.getElementById('chartAppointmentTrends'), {
    type: 'line',
    data: {
      labels: apptTrends.map(function (r) { return r.label; }),
      datasets: [
        { label: 'Total', data: apptTrends.map(function (r) { return r.total; }), borderColor: THEME.primary, backgroundColor: 'rgba(29,78,216,0.12)', fill: true, tension: 0.35 },
        { label: 'Completed', data: apptTrends.map(function (r) { return r.completed; }), borderColor: THEME.green, backgroundColor: 'rgba(22,163,74,0.12)', fill: true, tension: 0.35 }
      ]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: commonScales }
  });
...
<script>
let selBlood = {{ Illuminate\Support\Js::from(old('blood_group','')) }};
let selUrgency = {{ Illuminate\Support\Js::from(old('urgency','standard')) }};
