...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      <div class="tb-title">{{ __('Hospital Profile') }}</div>
    </div>
    <div class="tb-right">
  ...
  <div class="content">
    ...
    @if($hospital->is_verified)
      <div class="verify-banner verified">✓ {{ __('Your hospital account is verified.') }}</div>
    @else
      <div class="verify-banner pending">⏳ {{ __('Your hospital account is awaiting admin verification. You can still submit blood requests in the meantime.') }}</div>
    @endif

    <div class="card">
      <div class="card-t">{{ __('Hospital Details') }}</div>
      <div class="card-s">{{ __("Update your hospital's profile and contact information.") }}</div>
      ...
      <form method="POST" action="{{ route('hospital.profile.update') }}">
        ...
        <div class="form-grid">
          ...
          <div class="section-divider">{{ __('Change Password') }} ({{ __('optional') }})</div>

          <div class="field">
            <label>{{ __('New Password') }}</label>
            <input type="password" name="password" class="{{ $errors->has('password') ? 'is-invalid' : '' }}">
            <span class="hint">{{ __('Leave blank to keep your current password.') }}</span>
        ...
        <div class="actions">
          ...
          <a href="{{ route('hospital.dashboard') }}" class="btn-cancel">{{ __('Cancel') }}</a>
        </div>
      </form>
    </div>
  </div>
