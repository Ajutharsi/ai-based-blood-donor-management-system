<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .btn-back{padding:0.6rem 1.25rem;border:1px solid var(--gray-b);border-radius:8px;background:white;font-family:'DM Sans',sans-serif;font-size:0.85rem;color:var(--primary);cursor:pointer;text-decoration:none;font-weight:500;}
  ...
  .b-remove{background:var(--amber-bg);color:var(--amber);}
  .b-fulfillment{background:var(--primary-light);color:var(--primary-dark);}
  .b-threshold{background:var(--gray);color:var(--muted);}
  .change-pos{color:var(--green);font-weight:600;}
  .change-neg{color:var(--amber-dark);font-weight:600;}
...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      ...
      <div class="tb-title">{{ __('Inventory History') }}</div>
    </div>
    <div class="tb-right">
  ...
  <div class="content">
    ...
    <div class="page-hd">
      <div class="page-hd-left">
        <div class="page-label">{{ __('Reports') }}</div>
        <h1>{{ __('Inventory Change History') }}</h1>
      </div>
      <a href="{{ route('hospital.inventory.index') }}" class="btn-back">{{ __('← Back to Inventory') }}</a>
    </div>
    ...
    <div class="table-card">
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            @forelse($logs as $log)
              @php $change = $log->units_after - $log->units_before; @endphp
              <tr>
                <td><span class="blood-pill">{{ $log->blood_group }}</span></td>
                <td>
                  <span class="badge {{ $log->action === 'add' ? 'b-add' : ($log->action === 'remove' ? 'b-remove' : ($log->action === 'fulfillment' ? 'b-fulfillment' : 'b-threshold')) }}">
                    {{ ucfirst(str_replace('_', ' ', $log->action)) }}
                  </span>
                </td>
                <td>{{ $log->units_before }}</td>
                <td>{{ $log->units_after }}</td>
                <td class="{{ $change > 0 ? 'change-pos' : ($change < 0 ? 'change-neg' : '') }}">
                  {{ $change > 0 ? '+' : '' }}{{ $change }}
                </td>
                <td style="color:var(--muted);font-size:0.78rem;">{{ $log->note ?? '—' }}</td>
                <td style="font-size:0.75rem;color:var(--muted);white-space:nowrap;">{{ $log->created_at->diffForHumans() }}</td>
              </tr>
            @empty
              <tr>
                <td colspan="7">
                  <div class="empty-state">{{ __('No inventory changes recorded yet.') }}</div>
                </td>
              </tr>
            @endforelse
          </tbody>
      ...
      @if($logs->hasPages())
        <div class="pagination-wrap">
          <div class="page-info">
            {{ __('Showing') }} {{ $logs->firstItem() }}–{{ $logs->lastItem() }} {{ __('of') }} {{ $logs->total() }} {{ __('changes') }}
          </div>
