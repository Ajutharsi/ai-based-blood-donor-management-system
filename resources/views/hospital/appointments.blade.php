...
<div class="main">
  ...
  <div class="topbar">
    <div class="tb-left">
      ...
      <div class="tb-title">{{ __('Appointment Management') }}</div>
    </div>
    <div class="tb-right">
      <div class="hosp-pill">
  ...
  <div class="content">
    ...
    <div class="page-hd">
      <div class="page-label">{{ __('Appointments') }}</div>
      <h1>{{ __('Donation Appointment Management') }}</h1>
    </div>
    ...
    <div class="stat-row">
      <div class="sc">
        ...
        <div class="sc-num">{{ $today->count() }}</div><div class="sc-label">{{ __("Today's Appointments") }}</div>
      </div>
      <div class="sc">
        ...
        <div class="sc-num">{{ $upcoming->count() }}</div><div class="sc-label">{{ __('Upcoming') }}</div>
      </div>
      <div class="sc">
        <div class="sc-top"><div class="sc-icon ic-a"><svg viewBox="0 0 24 24"><path d="M12 8v4l3 3"/><circle cx="12" cy="12" r="10"/></svg></div></div>
        <div class="sc-num">{{ $pending->count() }}</div><div class="sc-label">{{ __('Awaiting Approval') }}</div>
      </div>
      <div class="sc">
        ...
        <div class="sc-num">{{ $appointments->where('status','completed')->count() }}</div><div class="sc-label">{{ __('Completed') }}</div>
      </div>
    </div>

    <div class="filter-tabs">
      <a href="{{ route('hospital.appointments.index') }}" class="ftab {{ !request('status') ? 'active' : '' }}">{{ __('All') }} ({{ $appointments->count() }})</a>
      <a href="{{ route('hospital.appointments.index', ['status' => 'pending']) }}" class="ftab {{ request('status') === 'pending' ? 'active' : '' }}">{{ __('Pending') }} ({{ $pending->count() }})</a>
      <a href="{{ route('hospital.appointments.index', ['status' => 'approved']) }}" class="ftab {{ request('status') === 'approved' ? 'active' : '' }}">{{ __('Approved') }}</a>
      <a href="{{ route('hospital.appointments.index', ['status' => 'completed']) }}" class="ftab {{ request('status') === 'completed' ? 'active' : '' }}">{{ __('Completed') }}</a>
    </div>
                  ...
                  <div class="actions">
                    @if($a->status === 'pending')
                      <form method="POST" action="{{ route('hospital.appointments.approve', $a->id) }}" style="margin:0;">
                        @csrf
                        <button type="submit" class="abtn approve">{{ __('Approve') }}</button>
                      </form>
                      <form method="POST" action="{{ route('hospital.appointments.reject', $a->id) }}" style="margin:0;">
                        @csrf
                        <button type="submit" class="abtn reject" onclick="return confirm('Reject this appointment?')">{{ __('Reject') }}</button>
                      </form>
                    @endif
                    @if($a->status === 'approved')
                      <form method="POST" action="{{ route('hospital.appointments.complete', $a->id) }}" style="margin:0;">
                        @csrf
                        <button type="submit" class="abtn complete" onclick="return confirm('Mark this appointment as completed and record the donation?')">{{ __('Complete') }}</button>
                      </form>
                    @endif
                    @if(in_array($a->status, ['pending', 'approved']))
                      <form method="POST" action="{{ route('hospital.appointments.reschedule', $a->id) }}" class="resched-form">
                        ...
                        <button type="submit" class="abtn">{{ __('Reschedule') }}</button>
                      </form>
                    @endif
                  </div>
                </td>
