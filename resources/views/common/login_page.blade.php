<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  :root{
    ...
    --warning:#D97706;--warning-dark:#92400E;--warning-light:#FFFBEB;
    --white:#fff;--off:#F8FAFC;--text:#1E293B;--muted:#64748B;
    --border:rgba(29,78,216,0.12);--gray:#F5F5F5;--gray-b:#E2E2E2;
  }
  ...
  /* LEFT */
  .left{background:var(--primary-dark);position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;padding:3.5rem 3rem;}
  ...
  .rc-donor{background:rgba(13,148,136,0.35);}
  ...
  .role-tab.active{background:var(--primary);color:white;font-weight:500;}
  .role-tab:not(.active):hover{background:var(--primary-light);color:var(--primary);}
  ...
  .rb-donor{background:#F0FDFA;color:#115E59;border:1px solid #99F6E4;}
  ...
  .input-wrap input:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(29,78,216,0.08);}
  .input-wrap input.is-invalid{border-color:var(--warning-dark);}
  ...
  .error-msg{font-size:0.72rem;color:var(--warning-dark);margin-top:4px;display:block;}
  ...
  .forgot{font-size:0.8rem;color:var(--primary);text-decoration:none;}

  /* ALERTS */
  .alert-error{background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;padding:0.8rem 1rem;margin-bottom:1.25rem;font-size:0.8rem;color:#92400E;}
  ...
  /* SUBMIT */
  .btn-submit{width:100%;padding:0.85rem;border:none;border-radius:9px;background:var(--primary);color:white;font-family:'DM Sans',sans-serif;font-size:0.95rem;font-weight:500;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;}
...
<nav>
  ...
  <div class="nav-right" style="display:flex;align-items:center;gap:14px;">
    ...
    <span>{{ __('New here?') }} <a href="{{ route('donor.register') }}">{{ __('Register as a Donor →') }}</a></span>
  </div>
</nav>
...
<div class="page">
  ...
  <div class="left">
    ...
    <div class="left-top">
      <div class="left-tag"><span></span> {{ __('Secure Portal') }}</div>
      <h2>{{ __('Welcome back to LifeLink') }}</h2>
      <p>{{ __('Sign in to your account. Choose your role and access your personalised dashboard.') }}</p>
    </div>
    <div class="role-cards">
      <div class="role-card active-card" id="card-donor" onclick="setRole('donor')">
        ...
        <div class="rc-info">
          ...
          <div class="rc-sub">{{ __('View donations & eligibility status') }}</div>
        </div>
      ...
      <div class="role-card" id="card-hospital" onclick="setRole('hospital')">
        ...
        <div class="rc-info">
          ...
          <div class="rc-sub">{{ __('Request blood & manage donors') }}</div>
        </div>
      ...
      <div class="role-card" id="card-admin" onclick="setRole('admin')">
        ...
        <div class="rc-info">
          <div class="rc-title">{{ __('Admin') }}</div>
          <div class="rc-sub">{{ __('Full system control & oversight') }}</div>
        </div>
  ...
  <div class="right">
    <div class="form-box">
      ...
      <div class="form-top">
        <div class="form-step" id="formStep">{{ __('Donor Portal') }}</div>
        <h1 id="formTitle">{{ __('Sign In') }}</h1>
        <p id="formSub">{{ __('Welcome back. Enter your credentials to continue.') }}</p>
      </div>
      ...
      <div class="role-badge rb-donor" id="roleBadge">
        ...
        <span id="roleBadgeText">{{ __('Signing in as a registered blood donor') }}</span>
      </div>

      {{-- ADMIN HINT --}}
      <div class="admin-hint" id="adminHint">
        <strong>{{ __('Admin access only') }}</strong>
        {{ __('This portal is restricted to authorised system administrators.') }}
      </div>
      ...
      <form method="POST" id="loginForm" action="{{ route('donor.login') }}">
        ...
        <div class="field-row">
          ...
          <a href="#" class="forgot">{{ __('Forgot password?') }}</a>
        </div>

        {{-- SUBMIT --}}
        <button type="submit" class="btn-submit" id="submitBtn">
          {{ __('Sign In to Donor Portal') }}
      ...
      <div class="register-link" id="registerLink">
        {{ __("Don't have an account?") }} <a href="{{ route('donor.register') }}">{{ __('Register as a Donor →') }}</a>
      </div>

      <div class="security-note">
        <svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
        {{ __("Your data is encrypted and protected under LifeLink's security policy.") }}
      </div>
...
const config = {
  donor: {
    step:       '{{ __('Donor Portal') }}',
    title:      '{{ __('Sign In') }}',
    sub:        '{{ __('Welcome back. Enter your credentials to continue.') }}',
    badge:      'rb-donor',
    badgeText:  '{{ __('Signing in as a registered blood donor') }}',
    btnText:    '{{ __('Sign In to Donor Portal') }}',
    showHospital: false,
    showAdmin:    false,
    register:   '{{ __("Don't have an account?") }} <a href="{{ route("donor.register") }}">{{ __('Register as a Donor →') }}</a>',
  },
  hospital: {
    step:       '{{ __('Hospital Portal') }}',
    title:      '{{ __('Hospital Login') }}',
    sub:        '{{ __('Access your dashboard to manage blood requests.') }}',
    badge:      'rb-hospital',
    badgeText:  '{{ __('Signing in as a registered hospital') }}',
    btnText:    '{{ __('Sign In to Hospital Portal') }}',
    showHospital: false,
    showAdmin:    false,
    register:   '{{ __('Not registered?') }} <a href="{{ route("hospital.register") }}">{{ __('Register your hospital →') }}</a>',
  },
  admin: {
    step:       '{{ __('Admin Portal') }}',
    title:      '{{ __('Admin Access') }}',
    sub:        '{{ __('Restricted area. Authorised personnel only.') }}',
    badge:      'rb-admin',
    badgeText:  '{{ __('Signing in as system administrator') }}',
    btnText:    '{{ __('Sign In as Admin') }}',
    showHospital: false,
    showAdmin:    true,
    register:   '{{ __('Need admin access?') }} <a href="#">{{ __('Contact IT support →') }}</a>',
  },
};
