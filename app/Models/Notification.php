...
class Notification extends Model
{
    ...
    public static function notify(string $userType, int $userId, string $title, string $message, string $type, ?int $relatedId = null, bool $logActivity = true): self
    {
        ...
        if ($logActivity) {
            ActivityLog::log(
                ...
                __(':title notification sent to :userType #:userId.', ['title' => $title, 'userType' => $userType, 'userId' => $userId]),
    ...
    // AI anomaly flags) that aren't tied to a single admin. Logs a single
    // activity entry for the broadcast itself rather than one per admin,
    // so a growing admin roster doesn't spam the activity log.
    public static function notifyAllAdmins(string $title, string $message, string $type, ?int $relatedId = null): void
    {
        foreach (Admin::pluck('id') as $adminId) {
            self::notify('admin', $adminId, $title, $message, $type, $relatedId, false);
        }

        ActivityLog::log(
            'notification', 'notification_sent',
            __(':title notification broadcast to all admins.', ['title' => $title]),
            'Notification', null
        );
    }
}
