...
class RegisterController extends Controller
{
    ...
    public function register(RegisterDonorRequest $request)
    {
        ...
        ActivityLog::record(
            'donor', $donor->id, $donor->full_name, 'registration', 'donor_registered',
            __(':name registered as a donor.', ['name' => $donor->full_name]),
            'Donor', $donor->id
        );
        ...
        if ($anomalyResult['is_anomaly'] ?? false) {
            Notification::notifyAllAdmins(
                __('AI anomaly alert'),
                __('AI flagged new donor :donor (:group) as a possible anomaly (score :score%).', [
