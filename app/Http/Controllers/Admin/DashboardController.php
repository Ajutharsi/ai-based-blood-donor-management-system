...
if ($aiAvailable) {
    try {
        ...
        // ── ADVANCED ANALYTICS (Chart.js data, last 6 months) ──
        $monthlyDonations = collect(range(5, 0))->map(function ($monthsAgo) {
            $month = now()->subMonths($monthsAgo);
            return [
                'label' => $month->format('M Y'),
                'count' => Donation::whereYear('donation_date', $month->year)
                    ->whereMonth('donation_date', $month->month)->count(),
            ];
        })->values();

        $bloodRequestTrends = collect(range(5, 0))->map(function ($monthsAgo) {
            $month = now()->subMonths($monthsAgo);
            return [
                'label'     => $month->format('M Y'),
                'total'     => BloodRequest::whereYear('created_at', $month->year)->whereMonth('created_at', $month->month)->count(),
                'fulfilled' => BloodRequest::whereYear('created_at', $month->year)->whereMonth('created_at', $month->month)->where('status', 'fulfilled')->count(),
            ];
        })->values();

        // Net inventory movement per month, derived from the audit log
        // (units_before/units_after per adjustment) rather than a point-in-
        // time snapshot, since historical stock levels aren't stored.
        $bloodInventoryTrends = collect(range(5, 0))->map(function ($monthsAgo) {
            $month = now()->subMonths($monthsAgo);
            $logs = BloodInventoryLog::whereYear('created_at', $month->year)->whereMonth('created_at', $month->month)->get();
            return [
                'label'   => $month->format('M Y'),
        ...
        $bloodGroupDistribution = collect($bloodGroups)->map(fn ($bg) => [
            'blood_group' => $bg,
            'count'       => Donor::where('blood_group', $bg)->count(),
        ])->values();

        $hospitalPerformance = Hospital::withCount(['bloodRequests as total_requests'])
            ->get()
            ->map(function (Hospital $h) {
                $fulfilled = BloodRequest::where('hospital_id', $h->id)->where('status', 'fulfilled')->count();
                return [
                    'hospital'  => $h->name,
                    'total'     => $h->total_requests,
                    'fulfilled' => $fulfilled,
                    'rate'      => $h->total_requests > 0 ? round($fulfilled / $h->total_requests * 100) : 0,
                ];
            })
            ->sortByDesc('total')
            ->take(8)
        ...
        $aiPredictionSummary = AiPrediction::select('prediction_type')
            ->selectRaw('count(*) as total, avg(confidence) as avg_confidence')
            ->groupBy('prediction_type')
            ->get()
            ->map(fn ($row) => [
                'type'           => ucfirst($row->prediction_type),
                'total'          => (int) $row->total,
                'avg_confidence' => $row->avg_confidence !== null ? round($row->avg_confidence, 1) : null,
            ])
            ->values();
